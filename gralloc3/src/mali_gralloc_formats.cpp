				    ...
				    pad_align_t *pad_align)
{
		...
		(alloc_format & MALI_GRALLOC_INTFMT_AFBC_BASIC))
	{
		...
		if (idx != -1)
		{
			...
			{
				short height_pad = (alloc_format & MALI_GRALLOC_INTFMT_AFBC_TILED_HEADERS) ? 16 : 32;
				*height += height_pad;

				pad_align->align.h = height_pad;
			}
		}
	}
	...
	if (producers & MALI_GRALLOC_PRODUCER_GPU)
	{
		...
		if (alloc_format & MALI_GRALLOC_INTFMT_AFBC_BASIC)
		{
			...
			*height = GRALLOC_ALIGN(*height, 16);
			pad_align->align.w = 16;
			pad_align->align.h = 16;
		}
	}
                                        ...
                                        const format_ip_support_t * const format)
{
	...
	if (producers & MALI_GRALLOC_PRODUCER_CAM)
	{
		if (format->id == MALI_GRALLOC_FORMAT_INTERNAL_Y16)
		{
			/* W/A for camera using Y16 with hwcomposer usage*/
			support &= format->cpu_wr;
		}
	}
	if (producers & MALI_GRALLOC_PRODUCER_VPU)
...
#if GRALLOC_SECURE_BUFFER_NO_AFBC == 1
                         ...
                         const bool map_to_internal)
{
	...
	else if (req_format == HAL_PIXEL_FORMAT_YCbCr_420_888)
	{
		...
		else if (usage == (GRALLOC_USAGE_HW_CAMERA_WRITE | GRALLOC_USAGE_HW_TEXTURE))
		{
			/* MALIDRV-386: W/A for android.hardware.camera2.cts.CameraExtensionSessionTest */
			base_format = HAL_PIXEL_FORMAT_YCrCb_420_SP;
		}
		else if (usage & (GRALLOC1_PRODUCER_USAGE_GPU_RENDER_TARGET | GRALLOC1_CONSUMER_USAGE_GPU_TEXTURE))
		{
			base_format = HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SP_M;
		}
		else
