...
#if defined(PRODUCT_VENDOR_T)
#include "ip_info/format_manager.h"
...
#define GRALLOC_VERBOSE_ALLOC "debug.gralloc.verbose_alloc"
...
static inline void dump_buffer_allocation(private_handle_t *hnd)
{
	if (property_get_bool(GRALLOC_VERBOSE_ALLOC, 0) == 0)
		return;

	android::hardware::IPCThreadState* ipc = android::hardware::IPCThreadState::self();
	uint64_t request_pid = static_cast<uint64_t>(ipc->getCallingPid());

	std::stringstream str {};
	str << "[buffer_alloc_info] request_pid: " << request_pid;

	str << ", requested: (" << hnd->width << "/" << hnd->height;
	str << "/0x" << std::hex << hnd->req_format << "/0x" << (hnd->producer_usage | hnd->consumer_usage) << std::dec << "), ";

	str << "allocated: " << hnd->backing_store_id << " (" << hnd->plane_info[0].alloc_width << "/";
	str << hnd->plane_info[0].alloc_height << "/0x" << std::hex << hnd->alloc_format << std::dec << "/";

	str << "fd_size{" << hnd->alloc_sizes[0] << ", " << hnd->alloc_sizes[1] << ", " << hnd->alloc_sizes[2] << "}/";
	str << "plane_size{" << hnd->plane_info[0].size << ", " << hnd->plane_info[1].size << ", " << hnd->plane_info[2].size << "})";

	ALOGD("%s", str.str().c_str());
	return;
}

static inline void dump_buffer_free(private_handle_t *hnd)
{
        if (property_get_bool(GRALLOC_VERBOSE_ALLOC, 0) == 0)
                return;
        ...
        std::stringstream str {};
        str << "[buffer_free_info] request_pid: " << request_pid;

        str << ", requested: (" << hnd->width << "/" << hnd->height;
        str << "/0x" << std::hex << hnd->req_format << "/0x" << (hnd->producer_usage | hnd->consumer_usage) << std::dec << "), ";

        str << "free: " << hnd->backing_store_id << " (" << hnd->plane_info[0].alloc_width << "/";
        str << hnd->plane_info[0].alloc_height << "/0x" << std::hex << hnd->alloc_format << std::dec << "/";
...
{
	const bool has_cpu_usage = usage & (GRALLOC_USAGE_SW_READ_MASK | GRALLOC_USAGE_SW_WRITE_MASK);
	const bool has_hw_usage = usage & ~(GRALLOC_USAGE_PRIVATE_MASK | GRALLOC_USAGE_SW_READ_MASK | GRALLOC_USAGE_SW_WRITE_MASK);
	...
	*size = 0;
	for (uint8_t plane = 0; plane < format.npln; plane++)
	{
		...
		else
		{
			...
			if (has_hw_usage) {
				if (format.is_rgb) {
					hw_align = RGB_BYTE_ALIGN_DEFAULT;
				}
				else if (format.is_yuv) {
					align_info align = m_format_manager.get_alloc_align(static_cast<PixelFormat>(format.id), usage);
					hw_align = align.stride_in_bytes;
				}
				else {
					hw_align = 0;
					if (format.id == MALI_GRALLOC_FORMAT_INTERNAL_RAW10 || format.id == MALI_GRALLOC_FORMAT_INTERNAL_RAW12) {
						hw_align = 16;
					}
				}
			}
			...
			/* Force align RAW10,RAW12 format to 16 byte stride */
#if defined(GRALLOC_SUPPORT_RAW10_RAW12)
			if (format.id == MALI_GRALLOC_FORMAT_INTERNAL_RAW10 || format.id == MALI_GRALLOC_FORMAT_INTERNAL_RAW12)
			{
				stride_align = 16;
			}
		...
		buffer_descriptor_t *bufDescriptor)
{
	...
	switch (format)
	{
...
#if defined(GRALLOC_SUPPORT_YCRCB_420_SP_ALIGN_16)
		...
		case HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SPN:
			...
			plane_count = setup_420_sp(w, h, fd_count, bufDescriptor->plane_info);
#else
			plane_count = setup_420_sp(w, h, fd_count, bufDescriptor->plane_info, EXT_SIZE);
#endif
			break;
...
int mali_gralloc_derive_format_and_size(buffer_descriptor_t * const bufDescriptor)
{
	...
	*/
	...
	{
		bufDescriptor->alloc_sizes[0] += (alloc_width*alloc_height);
	}

#if defined(JPEG_WA_ENABLED)
/*alloc extra 300K size for BLOB format to support thumbnailhistogram feature*/
	if (bufDescriptor->hal_format == HAL_PIXEL_FORMAT_BLOB)
	{
		bufDescriptor->alloc_sizes[0] += (300 * SIZE_1K);
	}
#endif
	return 0;
}
