                                    ...
                                    int* const height)
{
	...
	if (((producers & MALI_GRALLOC_PRODUCER_VPU) |  (consumers & MALI_GRALLOC_CONSUMER_VPU)) &&
		((alloc_format & MALI_GRALLOC_FORMAT_INTERNAL_P010) == MALI_GRALLOC_FORMAT_INTERNAL_P010))
	{
		*width = GRALLOC_ALIGN(*width, 16);
		*height = GRALLOC_ALIGN(*height, 16);
	}
                                ...
                                const uint64_t consumer_active_caps)
{
	...
	for (uint16_t i = 0; i < (uint16_t) num_formats; i++)
	{
	...
	for (uint16_t i = 0; i < (uint16_t) f_compat.size(); i++)
	{
