...
#define LOG_TAG "audio_hw_primary_abox"
//#define LOG_NDEBUG 0
...
static void deinit_route(struct audio_device *adev)
{
    ...
    struct audio_route *ar = (trinfo ? trinfo->aroute : NULL);
        ...
        bool set)
{
    ...
    /* Skip routing is output device is Aux-Digital */
    if (usage_id == AUSAGE_PLAYBACK_AUX_DIGITAL){
        ALOGVV("%s AUX_DIGITAL routing is skipped", __func__);
        return ret;
    }
    ...
    if (usage_type == AUSAGE_PLAYBACK) {
        ...
        ALOGW("%s Invalid Usage Type", __func__);
        return -EINVAL;
    }
        ...
        if (disable_cur_device) {
            if (ausage->usage_type == AUSAGE_PLAYBACK) {
                if (cur_device != DEVICE_NONE) {
                    ...
                    if (isCallMode(adev->usage_amode) ||isCallMode(cur_dev_amode)) {
                        ...
                        ALOGD("%s-%s: Current Active input Device is Disabled (%s)",
                            usage_table[usage_id], __func__, device_path_table[in_device]);
                        if (in_device != DEVICE_NONE && cur_dev_amode != AUSAGE_MODE_NORMAL) {
                            /* Disable current routed in-device path */
                            do_set_route(adev, in_usage_id, in_device, in_dev_amode, false);
                            //syncup_ausage_from_list(adev, NULL, AUSAGE_CAPTURE, in_device, DEVICE_NONE);
                        }
                    }
                }
            } else {
        ...
        if (enable_new_device) {
            if (ausage->usage_type == AUSAGE_PLAYBACK) {
                ...
                if (new_device != DEVICE_NONE) {
                    ...
                    if (isCallMode(adev->usage_amode)) {
                        device_type_t in_device = DEVICE_NONE;
                        audio_usage_id_t in_usage_id = AUSAGE_DEFAULT;
                        ...
                        if (in_device != DEVICE_NONE) {
                            ...
                            //syncup_ausage_from_list(adev, NULL, AUSAGE_CAPTURE, DEVICE_NONE, in_device);
                        }
                    }
                }
            } else {
...
static int do_open_btsco_loopback(
        struct audio_device *adev,
        unsigned int count )
{
    unsigned int loop = 0;
    ...
    /* Open MIC loopback PCM Devices to support internal BT */
    for (loop = 0; loop < count; loop++)
    {
        if (!adev->btmic_loop_pcminfo[loop]) {
            adev->btmic_loop_pcminfo[loop] = pcm_open(btmic_sound_device[loop][0], btmic_sound_device[loop][1],
                                                                            adev->btmic_loop_flags[loop], &adev->btmic_loop_pcmconfig[loop]);
            if (adev->btmic_loop_pcminfo[loop] && !pcm_is_ready(adev->btmic_loop_pcminfo[loop])) {
                /* pcm_open does always return pcm structure, not NULL */
                ALOGE("%s-%s: PCM Device is not ready(%s)!", "BTMIC_LOOPBACK",__func__, pcm_get_error(adev->btmic_loop_pcminfo[loop]));
                goto err_open;
            }

            snprintf(fn, sizeof(fn), "/dev/snd/pcmC%uD%u%c", btmic_sound_device[loop][0], btmic_sound_device[loop][1],
                                                adev->btmic_loop_flags[loop] & PCM_IN ? 'c' : 'p');

            ALOGI("[%s] Opened PCM Device(%s) configured Info: Channels(%d)  Smapling Rate(%d), Format: Default(%d)", __func__, fn,
            adev->btmic_loop_pcmconfig[loop].channels, adev->btmic_loop_pcmconfig[loop].rate, adev->btmic_loop_pcmconfig[loop].format);

            /* Start opened PCM Device */
            pcm_start(adev->btmic_loop_pcminfo[loop]);
...
err_open:
    ...
    {
        if (adev->btmic_loop_pcminfo[loop]) {
            pcm_stop(adev->btmic_loop_pcminfo[loop]);
            pcm_close(adev->btmic_loop_pcminfo[loop]);
            adev->btmic_loop_pcminfo[loop] = NULL;
        }
    }
...
static int do_close_btsco_loopback(
        struct audio_device *adev,
        unsigned int count __unused)
{
    ...
    for (loop = 0; loop < count; loop++)
    {
        if (adev->btmic_loop_pcminfo[loop]) {
            ...
            snprintf(fn, sizeof(fn), "/dev/snd/pcmC%uD%u%c", btmic_sound_device[loop][0], btmic_sound_device[loop][1],
                                                adev->btmic_loop_flags[loop] & PCM_IN ? 'c' : 'p');
            ALOGI("%s-%s: Closed PCM Device is %s", "BTMIC_LOOPBACK", __func__, fn);
        }
    }
...
static int do_start_voice_call(struct stream_out *out)
{
    ...
    unsigned int sound_device_tx;
    unsigned int sound_device_rx;
    ...
    sound_device_tx = VOICE_CALL_SOUND_DEVICE;
    sound_device_rx = VOICE_CALL_RX_DEVICE;
    ...
    if (isCPCallMode(adev->usage_amode) && voice_is_in_call(adev->voice)) {
        if (adev->pcm_voice_tx == NULL) {
            ...
            snprintf(fn, sizeof(fn), "/dev/snd/pcmC%uD%u%c", sound_card, sound_device_tx, 'p');
            adev->pcm_voice_tx = pcm_open(sound_card, sound_device_tx, PCM_OUT, &pcmconfig);
            if (adev->pcm_voice_tx && !pcm_is_ready(adev->pcm_voice_tx)) {
                ALOGE("%s-%s: PCM TX Device [ %s ] is not ready(%s)!", usage_table[out->ausage], __func__,
                                                                       fn, pcm_get_error(adev->pcm_voice_tx));
                goto err_tx;
            }
            ALOGI("%s-%s: Success to open PCM TX Device [ %s ]! ", usage_table[out->ausage], __func__, fn);

            /* Start TX Devices */
            if (adev->pcm_voice_tx) {
                pcm_start(adev->pcm_voice_tx);
                ALOGI("%s-%s: TX Success to Open, Start to Trigger PCM TX Device!", usage_table[out->ausage], __func__);
            }
        }

        if (adev->pcm_voice_rx == NULL) {
            /* Open Voice RX(Input) */
            snprintf(fn, sizeof(fn), "/dev/snd/pcmC%uD%u%c", sound_card, sound_device_rx, 'c');
            adev->pcm_voice_rx = pcm_open(sound_card, sound_device_rx, PCM_IN, &pcmconfig);
            if (adev->pcm_voice_rx && !pcm_is_ready(adev->pcm_voice_rx)) {
                ALOGE("%s-%s: PCM RX Device [ %s ] is not ready(%s)!", usage_table[out->ausage],__func__,
                                                                       fn, pcm_get_error(adev->pcm_voice_rx));
                goto err_rx;
            }
            ALOGI("%s-%s: Success to open PCM RX Device [ %s ]! ", usage_table[out->ausage], __func__, fn);

            /* Start RX Devices */
            if (adev->pcm_voice_rx) {
                pcm_start(adev->pcm_voice_rx);
                ALOGI("%s-%s: RX Success to Open, Start to Trigger PCM RX Device!", usage_table[out->ausage], __func__);
            }
        }

#ifdef SUPPORT_INTERNAL_BTSCO
        if (out->devices & AUDIO_DEVICE_OUT_ALL_SCO) {
            /* Open BT SCO loopback pcm nodes */
            do_open_btsco_loopback(adev, BTMIC_LOOPBACK_COUNT);
        }
#endif
        ret = 0;
    }
...
#ifdef SUPPORT_INTERNAL_BTSCO
    if (adev->btmic_loop_pcminfo[0] || adev->btmic_loop_pcminfo[1]) {
        /* close BT-SCO loopback pcm first */
        do_close_btsco_loopback(adev, BTMIC_LOOPBACK_COUNT);
    }
#endif
...
static void *offload_cbthread_loop(void *context)
{
    ...
    do {
        ...
        case OFFLOAD_MSG_WAIT_PARTIAL_DRAIN:
            ...
            do_stop_output_stream(out, true);
        ...
        struct audio_config *config)
{
    ...
    ALOGV("Check PCM Channel Count: Default(%d), Request(%d)",
                pcmconfig->channels, audio_channel_count_from_out_mask(config->channel_mask));
    pcmconfig->channels = audio_channel_count_from_out_mask(config->channel_mask);

    ALOGV("Check PCM Smapling Rate: Default(%d), Request(%d)", pcmconfig->rate, config->sample_rate);
    pcmconfig->rate = config->sample_rate;

    ALOGV("Check PCM Format: Default(%d), Request(%d)",
...
static void check_and_set_pcm_config_capture(
        struct pcm_config *pcmconfig,
        struct audio_config *config)
{
    /* Need to check which values are selected when requested configuration
    ...
    ALOGV("Check PCM Channel Count: Default(%d), Request(%d)",
                pcmconfig->channels, audio_channel_count_from_in_mask(config->channel_mask));
    // A-Box supports only Stereo Capture
    pcmconfig->channels = audio_channel_count_from_in_mask(AUDIO_CHANNEL_IN_STEREO);
    ALOGV("PCM Channel Count set to Stereo forcibly");
...
static int do_close_input_stream(struct stream_in *in)
{
    ...
    pthread_mutex_lock(&adev->inpcm_lock);
...
#ifdef SUPPORT_INTERNAL_BTSCO
        if (in->devices == AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
            ALOGI("%s-%s: Device 0x%x", usage_table[in->ausage], __func__, in->devices);
            do_close_btsco_loopback(adev, BTMIC_LOOPBACK_COUNT);
        }
#endif
    }
    ...
    pthread_mutex_unlock(&adev->inpcm_lock);

    if ( in->ausage != AUSAGE_CAPTURE_VOICE_CALL) {
        /* Reset Routing Path */
        /* Have to keep devices information to restart without calling set_parameter() */
...
static int do_read_buffer(struct stream_in *in, void* buffer, size_t bytes)
{
    ...
    if (in->pcminfo) {
        if (in->ausage == AUSAGE_CAPTURE_VOICE_CALL) {
            src_bytes = pcm_frames_to_bytes(in->pcminfo, in->pcmconfig.period_size);
            ret = pcm_read(in->pcminfo, (void*)(in->resample_buf), (unsigned int)src_bytes);
        } else if (in->need_monoconversion && in->pcmbuffer)
            ret = pcm_read(in->pcminfo, (void*)(in->pcmbuffer), (unsigned int)bytes);
        else
        ...
        if (ret == 0) {
            ...
            if (in->ausage == AUSAGE_CAPTURE_VOICE_CALL) {
                int data_mono;
                short *vc_buf = (short *)in->resample_buf;
                size_t i = 0;
                for (i; i < in->pcmconfig.period_size; i++){
                    if (in->source == AUDIO_SOURCE_VOICE_UPLINK) {
                        data_mono = (int16_t)((int32_t)*(vc_buf+2*i+1)); //Tx
                    } else if (in->source == AUDIO_SOURCE_VOICE_DOWNLINK){
                        data_mono = (int16_t)((int32_t)*(vc_buf+2*i)); // Rx
                    } else {
                        data_mono = (int16_t)(((int32_t)*(vc_buf+2*i) + (int32_t)*(vc_buf+2*i+1)) >> 1); //mix Rx/Tx
                    }

                    *(vc_buf + 2 * i) = (int16_t)data_mono;
                    *(vc_buf + 2 * i + 1) = (int16_t)data_mono;
                }

                if (in->resampler) {
                    size_t inframe_cnt = pcm_bytes_to_frames(in->pcminfo, src_bytes); //bytes/bytes_per_sample;
                    size_t outframe_cnt = bytes /audio_stream_in_frame_size(&in->stream);
                    in->resampler->resample_from_input(in->resampler, in->resample_buf,
                                                                    &inframe_cnt, (int16_t *)buffer, &outframe_cnt);
                    ALOGVV("%s-%s: Resample Done (%u bytes) inframe count %zu outframe count %zu", usage_table[in->ausage],
                                            __func__, (unsigned int)bytes, inframe_cnt, outframe_cnt);
                    read = bytes;
                } else
                    ALOGE("%s-%s: Resampler not created", usage_table[in->ausage], __func__);
            } else if (in->need_monoconversion  && in->pcmbuffer) {
                read = bytes / 2;
                adjust_channels((const void *)(in->pcmbuffer), 2, (void *)buffer, 1, 2, bytes); // Stereo to Mono
            } else
                read = bytes;

            in->read += read / (in->pcmconfig.channels * sizeof(short)); // convert to frame unit
...
static int check_input_parameters(struct audio_config *config)
{
    int ret = 0;
...
static size_t get_input_buffer_size(uint32_t sample_rate, audio_format_t format, audio_channel_mask_t channel_mask, struct audio_device *adev)
{
    size_t size = 0;

    if(isCPCallMode(adev->usage_amode)) {
        // Number of Sample during duration
        size = (sample_rate * AUDIO_VOICECALL_CAPTURE_PERIOD_DURATION_MSEC) / 1000;
    } else {
        // Number of Sample during duration
        size = (sample_rate * AUDIO_CAPTURE_PERIOD_DURATION_MSEC) / 1000;
    }

    // Multiply Audio Bytes and Channel Count
    size *= audio_bytes_per_sample(format) * audio_channel_count_from_in_mask(channel_mask);
    ...
    if (ret >= 0) {
        ...
        if (req_device != AUDIO_DEVICE_NONE) {
            if (((req_device & AUDIO_DEVICE_OUT_ALL_SCO) || (out->devices & AUDIO_DEVICE_OUT_ALL_SCO))
                    && (req_device !=  out->devices)) {
                    need_vcpcm_ctrl = true;
            }
            ...
            if (need_routing) {
                if (need_vcpcm_ctrl) {
                    /* Close Voice call PCM nodes before routing to BT SCO */
                    if (adev->voice && voice_is_in_call(adev->voice)) {
                        do_stop_voice_call(adev);
                        ALOGD("device-%s: #***# Stopped CP Voice Call for BT SCO Routing #***#", __func__);
                    }
                }
                ...
                if (output_drives_call(adev, out) && isCPCallMode(adev->usage_amode)) {
                    if (adev->voice) {
                        ...
                        if (voice_is_in_call(adev->voice)) {
                            ...
                            if (adev->pcm_voice_tx == NULL && adev->pcm_voice_rx == NULL) {
                                do_start_voice_call(out);
                                ALOGD("%s-%s: *** Started CP Voice Call ***", usage_table[out->ausage],__func__);
                            } else
                                ALOGV("%s-%s: CP Voice Call PCM Nodes", usage_table[out->ausage], __func__);
                        }
                    }
                }
            }
        ...
        size_t bytes)
{
    ...
    /* Skip other sounds except HDMI */
    if (out->ausage != AUSAGE_PLAYBACK_AUX_DIGITAL &&
        out->devices ==AUDIO_DEVICE_OUT_AUX_DIGITAL) {
        ALOGVV("%s-%s: Skip device open", usage_table[out->ausage], __func__);
        return bytes;
    }
...
static size_t in_get_buffer_size(const struct audio_stream *stream)
{
    ...
    /* Calcurate size from Requested PCM Configuration */
    size = get_input_buffer_size(in->sample_rate, in->format, in->channel_mask, adev);
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        /* AudioFlinger wants to change Audio Routing to some device */
        req_device = atoi(value);
        if (req_device != 0 && in->ausage != AUSAGE_CAPTURE_VOICE_CALL) {
        ...
        size_t bytes)
{
    ...
    if (in->ausage == AUSAGE_CAPTURE_VOICE_CALL &&
        in->sstate == STATE_ERROR) {
        ALOGVV("%s-%s: VoiceCall Stopped sent dummy buffer after waiting", usage_table[in->ausage], __func__);
        usleep((in->pcmconfig.period_size * 1000)/in->pcmconfig.rate * 1000); //as Voice call recording interval is 20ms
        goto voicecallcap_err;
    }
    ...
    if (in->sstate == STATE_STANDBY) {
        ...
        if (in->ausage == AUSAGE_CAPTURE_VOICE_CALL) {
            if (in->resampler) {
                ALOGW("%s-%s: resampler should be re-created", usage_table[in->ausage], __func__);
                release_resampler(in->resampler);
                in->resampler = NULL;
            }
             if (in->sample_rate != in->pcmconfig.rate) {
                ret = create_resampler(in->pcmconfig.rate,
                                                 in->sample_rate,
                                                 2,
                                                 ...
                                                 &in->resampler);
                if (ret !=0) {
                    ALOGE("%s-%s: Failed to create resampler", usage_table[in->ausage], __func__);
                } else {
                    ALOGD("%s-%s: resampler created in-samplerate %d out-samplereate %d", usage_table[in->ausage],  __func__,
                                in->pcmconfig.rate, in->sample_rate);
        ...
        if (ret != 0) {
            ALOGE("%s-%s: Fail to open Input Stream!", usage_table[in->ausage], __func__);
        } else {
            in->sstate = STATE_IDLE;
            ALOGI("%s-%s: Transit to Idle", usage_table[in->ausage], __func__);
        }
    ...
    if (in->sstate == STATE_PLAYING) {
        ...
        if (ret < 0 && in->ausage == AUSAGE_CAPTURE_VOICE_CALL) {
            ALOGI("%s-%s: VoiceCall pcm_read failed", usage_table[in->ausage], __func__);
            in->sstate = STATE_ERROR;
            goto voicecallcap_err;
        }
    }
...
voicecallcap_err:
    ALOGVV("%s-%s: sent dummy buffer when read failed", usage_table[in->ausage], __func__);
        ...
        *time = 0;
        ...
        if (in->pcminfo) {
            ...
            if (ret < 0) {
                ...
                if (signed_frames >= 0) {
                    *frames = (uint64_t)signed_frames;
                    *time = (int64_t)(((int64_t)timestamp.tv_sec * 1000000000LL) + timestamp.tv_nsec); // Nano Seconds Unit
                    ret = 0;
                }
            }
        }
        ...
        const char *address __unused)
{
    ...
    *stream_out = NULL;
    ...
    if ((out->flags & AUDIO_OUTPUT_FLAG_PRIMARY) != 0) {
        ...
        if ((out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) != 0) {
...
//                out->comprconfig.codec->sample_rate = compress_get_alsa_rate(config->sample_rate);
    ...
    } else if ((out->devices & AUDIO_DEVICE_OUT_AUX_DIGITAL) != 0) {
        /* Sub-Case: Playback with Aux Digital */
        ALOGD("device-%s: Requested to open AUX-DIGITAL output", __func__);
        out->ausage = AUSAGE_PLAYBACK_AUX_DIGITAL;

        /* Set PCM Configuration */
        out->pcmconfig = pcm_config_dp_buffer;
        check_and_set_pcm_config(&out->pcmconfig, config);
    } else {
            ...
            struct audio_stream_out *stream)
{
    ...
    if (out) {
         id = out->ausage;
        ALOGD("device-%s: Audio Usage(%s)", __func__, usage_table[id]);
...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    ...
    if(volume < 0.0f || volume > 1.0f) {
        ALOGD("%s invalid volume:%f",__func__, volume);
        return 0;
    }
...
static int adev_set_mode(struct audio_hw_device *dev, audio_mode_t mode)
{
    ...
    ALOGD("device-%s: enter with mode = %d, previous mode = %d", __func__, mode, adev->amode);

    pthread_mutex_lock(&adev->lock);
    if (adev->amode != mode) {
        /* Set Audio Mode to Kernel */
        val = (int)mode;
        ctrl = mixer_get_ctl_by_name(adev->mixerinfo, ABOX_AUDIOMODE_CONTROL_NAME);
        if (ctrl) {
            ret = mixer_ctl_set_value(ctrl, 0,val);
            if (ret != 0)
                ALOGE("device-%s: Fail to set AudioMode to Kernel", __func__);
            else
                ALOGI("device-%s: Set AudioMode to Kernel", __func__);
        } else {
            ALOGE("device-%s: Cannot find AudioMode Mixer Control", __func__);
        }
...
{
    struct audio_device *adev = (struct audio_device *)dev;
    size_t size = get_input_buffer_size(config->sample_rate, config->format, config->channel_mask, adev);

    ALOGD("device-%s: exited with %d Bytes from SR(%d), Format(%d), Mask(%d)", __func__, (int)size,
        ...
        audio_source_t source)
{
    ...
    *stream_in = NULL;
    ...
    /* Check required configuration for HW Constraint */
    if (check_input_parameters(config) != 0) {
        ALOGD("device-%s: Return with proposed configuration!", __func__);
        return -EINVAL;
    }
    ...
    in->pcminfo = NULL;
    in->need_monoconversion = false;

    /* Set basic configuration from devices */
   if ((in->flags & AUDIO_INPUT_FLAG_FAST) != 0){
        /* Case: Capture For Fast Recording */
        ALOGD("device-%s: Requested open Low Latency input", __func__);

        in->ausage = AUSAGE_CAPTURE_LOW_LATENCY;

        /* Set PCM Configuration */
        in->pcmconfig = pcm_config_low_latency_capture;// Fast PCM Configuration
        check_and_set_pcm_config_capture(&in->pcmconfig, config);
    } else {
        if (adev->amode == AUDIO_MODE_IN_CALL) {
            /* Case: Capture For Voicecall Recording */
            ALOGD("device-%s: Requested open Voicecall recording with L-rx/R-tx input", __func__);

            in->ausage = AUSAGE_CAPTURE_VOICE_CALL;

            /* Set PCM Configuration */
            in->pcmconfig = pcm_config_vc_rec;// voicecall Record PCM Configuration

            in->resample_buf = (int16_t*)malloc(pcm_config_vc_rec.period_size *
                                                              pcm_config_vc_rec.channels *
                                                              sizeof(int16_t));
            if (!in->resample_buf) {
                ret = -ENOMEM;
            }
        } else {
            /* Case: Capture For Primary Recording */
            ALOGD("device-%s: Requested open Primary input", __func__);

            in->ausage = AUSAGE_CAPTURE_PRIMARY;

            /* Set PCM Configuration */
            in->pcmconfig = pcm_config_primary_capture;// Default PCM Configuration
            check_and_set_pcm_config_capture(&in->pcmconfig, config);
        }
    }

    /* Check and Set Mono Conversion */
    /* A-Box supports only stereo capture even request mono */
    if (channel_count == 1 && in->ausage != AUSAGE_CAPTURE_VOICE_CALL) {
        ALOGD("device-%s: This stream need to convert to Mono", __func__);
        in->need_monoconversion = true;
        in->pcmbuffer = malloc(in->pcmconfig.period_size * in->pcmconfig.channels * sizeof(short));
        if (!in->pcmbuffer) {
            ret = -ENOMEM;
            goto err_open;
        }
        ...
        struct audio_stream_in *stream)
{
    ...
    if (in) {
        ...
        if (in->ausage == AUSAGE_CAPTURE_VOICE_CALL) {
            if (in->resampler) {
                ALOGD("device-%s: Audio Usage(%s) release resampler", __func__, usage_table[id]);
                release_resampler(in->resampler);
                in->resampler = NULL;
            }
...
static int adev_dump(const audio_hw_device_t *device, int fd)
{
    ...
    char calliopedbgpath[] = "/sys/kernel/debug/abox/log-00";
    int dbg_fd = -1;
    char *buffer = NULL;
    ...
    /* allocate 4Kbytes  buffer */
    buffer = malloc(CALLIOPE_LOG_BUFFERSIZE);
    if (!buffer) {
        ALOGE("device-%s: Failed to allocate buffer", __func__);
        return -ENOMEM;
    }

    /* Get A-Box GPR register dump */
    dbg_fd = open(ABOX_GPR_DUMP_PATH, O_RDONLY | O_NONBLOCK);
    if (dbg_fd < 0) {
        strerror_r(errno, buffer, 256);
        ALOGE("device-%s: Failed to Open %s - %s", __func__,
            ABOX_GPR_DUMP_PATH, buffer);
        free(buffer);
        return -EINVAL;
    }

    bytes_read = read(dbg_fd, buffer, CALLIOPE_LOG_BUFFERSIZE);
    ALOGI("device-%s: Calliope Log bytes-read: %zd", __func__, bytes_read);
    if (bytes_read > 0)
    ...
    snprintf(buffer, CALLIOPE_LOG_BUFFERSIZE, "Calliope Firmware Log Messages\n");
    write(fd, buffer, strlen(buffer));
    snprintf(buffer, CALLIOPE_LOG_BUFFERSIZE, "==============================\n");
    write(fd, buffer, strlen(buffer));
    /* Get calliope firmware log messages */
    dbg_fd = open(calliopedbgpath, O_RDONLY | O_NONBLOCK);
    if (dbg_fd < 0) {
        strerror_r(errno, buffer, 256);
        ALOGE("device-%s: Failed to Open %s - %s", __func__,
            calliopedbgpath, buffer);
        free(buffer);
        return -EINVAL;
    }
    ...
    do {
        ...
        if (bytes_read > 0)
            write(fd, buffer, bytes_read);
        ALOGV("device-%s: Calliope Log bytes-read: %zd", __func__, bytes_read);
    } while (bytes_read > 0);
...
static int adev_close(hw_device_t *device)
{
    ...
    if (adev) {
        ...
        pthread_mutex_unlock(&adev->lock);
        pthread_mutex_destroy(&adev->inpcm_lock);
        ...
        hw_device_t** device)
{
    ...
    pthread_mutex_init(&adev->inpcm_lock, (const pthread_mutexattr_t *) NULL);
