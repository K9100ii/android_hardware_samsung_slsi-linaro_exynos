...
#include "../voicemanager/ww/voice_manager.h"
...
typedef enum {
    ...
    AUSAGE_CAPTURE_PRIMARY,            // For Primary Input Profile
    AUSAGE_CAPTURE_LOW_LATENCY,        // For Fast Input Profile
    AUSAGE_CAPTURE_VOICE_CALL,         // For Primary VoiceCall Input Profile
 ...
 **/
typedef enum {
    STATE_ERROR     = -1,  // Stream unable to get pcm data from device.
...
typedef enum {
    ...
    DEVICE_MAIN_MIC,                    // Main Stereo MIC
    DEVICE_SUB_MIC,                     // Second Mono MIC
    DEVICE_HEADSET_MIC,
...
struct stream_in {
    ...
    /* Voice call recording
        recording scheme: voice call rx/tx data is recorded separately as stereo data
        CP downlink data : L-channel contains CP Rx data
        CP Uplink data: R-channel CP Tx data */
