...
struct mixer            *gRvbMixerHandle = NULL;
struct mixer_ctl        *gRvbMixerParamCtl = NULL;
struct mixer_ctl        *gRvbMixerCtxCtl = NULL;
...
extern "C" int EffectGetDescriptor(const effect_uuid_t *uuid,
    ...
    /* open mixer control */
    if (!gRvbMixerHandle) {
        gRvbMixerHandle = mixer_open(MIXER_CARD);
        if (gRvbMixerHandle) {
            /* Get required param & context controls from mixer */
            gRvbMixerParamCtl = mixer_get_ctl_by_name(gRvbMixerHandle, OFFLOAD_RVB_PARAM_CTL_NAME);
            gRvbMixerCtxCtl = mixer_get_ctl_by_name(gRvbMixerHandle, OFFLOAD_RVB_CTX_CTL_NAME);
            if (!gRvbMixerParamCtl || !gRvbMixerCtxCtl) {
                ALOGE("%s: mixer_get_ctl_by_name failed", __func__);
                mixer_close(gRvbMixerHandle);
                gRvbMixerHandle = NULL;
                return -EINVAL;
            }
        } else{
...
LVREV_ReturnStatus_en Offload_SetEffect_ControlParameters(ReverbContext *pContext){
    LVREV_ReturnStatus_en     LvmStatus = LVREV_SUCCESS;          /* Function call status */
    int32_t param[OFFLOAD_RVB_PARAM_MAX] = {0};
    int i, ret;
    ...
    if (pContext->OffloadEnabled) {
        ...
        if (pContext->param_mixerCtl) {
            ...
            if (ret) {
                ...
                for (i=0; i < OFFLOAD_RVB_PARAM_MAX; i++) {
...
LVREV_ReturnStatus_en Offload_SetEffect_ContextParameters(ReverbContext *pContext){
    LVREV_ReturnStatus_en     LvmStatus = LVREV_SUCCESS;          /* Function call status */
    int32_t param[OFFLOAD_RVB_CTX_MAX] = {0};
    int i, ret;
    ...
    if (pContext->OffloadEnabled) {
        ...
        if (pContext->ctx_mixerCtl) {
            ...
            if (ret) {
                ...
                LvmStatus = LVREV_SUCCESS;
                for (i=0; i < OFFLOAD_RVB_CTX_MAX; i++) {
...
void Reverb_free(ReverbContext *pContext){
    ...
    pContext->mixerHandle = NULL;
    pContext->param_mixerCtl = NULL;
    pContext->ctx_mixerCtl = NULL;
}    /* end Reverb_free */
...
//    int status;
    ...
    if (gRvbMixerHandle) {
        pContext->mixerHandle = gRvbMixerHandle;
        pContext->param_mixerCtl = gRvbMixerParamCtl;
        pContext->ctx_mixerCtl = gRvbMixerCtxCtl;
    } else {
...
int Reverb_process(effect_handle_t   self __unused,
