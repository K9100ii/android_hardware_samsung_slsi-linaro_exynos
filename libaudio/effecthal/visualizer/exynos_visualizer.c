...
#ifdef SUPPORT_OFFLOAD_VISUALIZER_MIXER
/* mixer controls for loopback path */
#define MIXER_CARD 0
#define MIXER_CTL_PIFS0_NAME    "ABOX RECP PIFS0"
#define MIXER_CTL_RSRC0_NAME    "ABOX RSRC0"

static struct mixer         *gVlzMixerHandle = NULL;
static struct mixer_ctl     *gVlzMixerPifs0Ctl = NULL;
static struct mixer_ctl     *gVlzMixerRsrc0Ctl = NULL;
#endif
...
#ifdef SUPPORT_OFFLOAD_VISUALIZER_MIXER
int visualizer_loopback_enable(bool enable)
{
    int ret = 0;

    ALOGV("\t %s", __func__);

    if (gVlzMixerPifs0Ctl && gVlzMixerRsrc0Ctl) {
        ALOGD("\t %s Visualizer LoopBack configuration",(enable ? "Enable" : "Disable"));
        /* Turn on ABOX PIFS0 */
        ret = mixer_ctl_set_value(gVlzMixerPifs0Ctl, 0, (enable ? 1 : 0));
        if (ret) {
            ALOGE("%s: mixer_ctl_set_ return error(%d)", __func__, ret);
        } else {
            /* SIFS0 output is loopback to RSCR0 */
            ret = mixer_ctl_set_enum_by_string(gVlzMixerRsrc0Ctl, (enable ? "SIFS0" : "RESERVED"));
            if (ret) {
                ALOGE("%s: mixer_ctl_set_ return error(%d)", __func__, ret);
            }
        }
    } else {
        ALOGE("%s: Failed to open mixer controls", __func__);
    }
    ...
    if (out_ctxt == NULL) {
        ...
        if (list_empty(&active_outputs_list)) {
            if (thread_status == 0) {
                ...
                thread_status = -1;
#ifdef SUPPORT_OFFLOAD_VISUALIZER_MIXER
                visualizer_loopback_enable(false);
#endif
            }
        }
        free(out_ctxt);
        ...
        effect_descriptor_t *descriptor)
{
    ...
    /* open mixer control */
    if (!gVlzMixerHandle) {
        gVlzMixerHandle = mixer_open(MIXER_CARD);
        if (gVlzMixerHandle) {
            /* Get required control from mixer */
            gVlzMixerPifs0Ctl = mixer_get_ctl_by_name(gVlzMixerHandle, MIXER_CTL_PIFS0_NAME);
            gVlzMixerRsrc0Ctl = mixer_get_ctl_by_name(gVlzMixerHandle, MIXER_CTL_RSRC0_NAME);
            if (!gVlzMixerPifs0Ctl || !gVlzMixerRsrc0Ctl) {
                ALOGE("%s: mixer_get_ctl_by_name failed", __func__);
                mixer_close(gVlzMixerHandle);
                gVlzMixerHandle = NULL;
                ret = -EINVAL;
            }
        } else{
