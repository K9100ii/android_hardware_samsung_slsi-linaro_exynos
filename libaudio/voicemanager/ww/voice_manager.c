...
int voice_open(struct voice_manager *voice)
{
    ...
    if (!voice->state_call) {
        /* When RilClient was not opened, try to open again */
        if (voice->state_open == false) {
            ALOGD("%s: RilClient is not opened yet! Retry!", __func__);
            if (voice->rilc.ril_open_client) {
                ret = voice->rilc.ril_open_client();
                if (ret != 0)
                    ALOGE("%s: Failed to open RIL AudioClient!", __func__);
                else {
                    ALOGD("%s: Succeeded in opening RIL AudioClient!", __func__);
                    voice->state_open = true;
                }
            } else
        ...
        if (voice->state_open) {
            ...
            ret = 0;
            ALOGD("%s: Transit to IN_CALL!", __func__);
        }
    }
...
int voice_close(struct voice_manager *voice)
{
    if (voice->state_call) {
        ...
        ALOGD("%s: Transit to NOT_IN_CALL!", __func__);
    }
...
int voice_set_callback(struct voice_manager * voice, void * callback_func)
{
    ...
    if (voice->rilc.ril_register_callback) {
        ...
        if (ret == 0) {
            ALOGD("%s: Succeeded in registering Callback Function!", __func__);
    ...
    if (voice) {
        if (access(RIL_CLIENT_LIBPATH, R_OK) == 0) {
            ...
            if (voice->rilc.handle) {
                ...
                ALOGD("%s: Succeeded in opening RIL AudioClient Interface!", __func__);
            } else {
                ALOGE("%s: Failed to open RIL AudioClient Interface(%s)!", __func__, RIL_CLIENT_LIBPATH);
                goto open_err;
            }
        } else {
            ALOGE("%s: Failed to access RIL AudioClient Interface(%s)!", __func__, RIL_CLIENT_LIBPATH);
            goto open_err;
        }

        voice->state_open = false;
        voice->state_call = false;
