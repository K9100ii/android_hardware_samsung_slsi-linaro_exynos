 ...
 */
 ...
 /* #define LOG_NDEBUG 0 */
#define LOG_TAG "ExynosJpegEncoderForCamera"
...
ExynosJpegEncoderForCamera::ExynosJpegEncoderForCamera(__unused bool bBTBComp)
{
    m_flagCreate = false;
    m_jpegMain = NULL;
    m_jpegThumb = NULL;
    ...
    initJpegMemory(&m_stThumbTempBuf, MAX_IMAGE_PLANE_NUM);
    initJpegMemory(&m_stThumbOutBuf, MAX_IMAGE_PLANE_NUM);

    m_jpegMainEncodeThread = new JpegMainEncodeThread(this, &ExynosJpegEncoderForCamera::m_JpegMainEncodeThreadFunc, "JpegMainEncodeThread");
...
int ExynosJpegEncoderForCamera::create(void)
{
    ...
    m_stThumbTempBuf.ionClient = m_ionJpegClient;
...
int ExynosJpegEncoderForCamera::destroy(void)
{
    ...
    if (m_jpegThumb != NULL) {
        ...
        freeJpegMemory(&m_stThumbTempBuf, MAX_IMAGE_PLANE_NUM);
...
int ExynosJpegEncoderForCamera::setQuality(const unsigned char q_table[])
{
    if (m_flagCreate == false)
        return ERROR_NOT_YET_CREATED;

#ifdef DISABLE_JPEG_QUALITY_SETTING
    ALOGW("WARN(%s[%d]):DISABLE_JPEG_QUALITY_SETTING, so, just return 0", __FUNCTION__, __LINE__);
    return 0;
#else
    return m_jpegMain->setQuality(q_table);
#endif
}
                              ...
                              bool useMainbufForThumb)
{
    ...
    if (exifInfo->enableThumb) {
        ...
        ALOGD("DEBUG(%s:%d): thumbnail size=%d", __FUNCTION__, __LINE__, thumbSize);
    }
...
int ExynosJpegEncoderForCamera::encodeThumbnail(unsigned int *size, bool useMain)
{
    ...
    if (useMain) {
        ...
        csc_set_hw_property(m_exynosThumbCSC, CSC_HW_PROPERTY_FIXED_NODE, m_extScalerNum);
        /********************/
            ...
            if (m_jpegMain->checkInBufType() & JPEG_BUF_TYPE_DMA_BUF) {
                ...
                if ((m_exynosThumbCSC && ALIGN_DOWN(iTempWidth, 16) == iTempWidth)
                    ...
                    csc_set_src_format(m_exynosThumbCSC,
                        ALIGN_DOWN(iTempWidth,16), ALIGN_DOWN(iTempHeight,2),
                        0, 0, ALIGN(iTempWidth, 16), ALIGN_DOWN( iTempHeight,2),
                        V4L2_PIX_2_HAL_PIXEL_FORMAT(iTempColorformat), 1);
                    ...
                    if (csc_convert(m_exynosThumbCSC) != 0)
                        ALOGE("ERR(%s[%d]) dmabuf : csc_convert() from gralloc to callback fail", __FUNCTION__, __LINE__);
                } else if ((m_exynosThumbCSC && ALIGN_DOWN(iTempWidth, 16) == iTempWidth)
                    && ((iTempWidth > (m_thumbnailW * 16)) || (iTempHeight > (m_thumbnailH * 16)))) {
                    int m_thumbnailW_temp = m_thumbnailW;
                    int m_thumbnailH_temp = m_thumbnailH;
                    if (iTempWidth > (m_thumbnailW * 16)) {
                        m_thumbnailW_temp = m_thumbnailW * 4;
                    }
                    if (iTempHeight > (m_thumbnailH * 16)) {
                        m_thumbnailH_temp = m_thumbnailH * 4;
                    }

                    m_stThumbTempBuf.iSize[0] = sizeof(char) * m_thumbnailW_temp * m_thumbnailH_temp * THUMBNAIL_IMAGE_PIXEL_SIZE;
                    if (allocJpegMemory(&m_stThumbTempBuf, MAX_IMAGE_PLANE_NUM) != ERROR_NONE)
                        return ERROR_MEM_ALLOC_FAIL;
                    ...
                    csc_set_src_format(m_exynosThumbCSC,
                        ...
                        V4L2_PIX_2_HAL_PIXEL_FORMAT(iTempColorformat), 1);
                    csc_set_dst_format(m_exynosThumbCSC,
                        m_thumbnailW_temp, m_thumbnailH_temp,
                        0, 0, m_thumbnailW_temp, m_thumbnailH_temp,
                        V4L2_PIX_2_HAL_PIXEL_FORMAT(iTempColorformat), 1);
                    csc_set_src_buffer(m_exynosThumbCSC,
                        (void**)iMainInputBuf, CSC_MEMORY_DMABUF);
                    csc_set_dst_buffer(m_exynosThumbCSC,
                        (void**)m_stThumbTempBuf.ionBuffer, CSC_MEMORY_DMABUF);
                    if (csc_convert(m_exynosThumbCSC) != 0)
                        ALOGE("ERR(%s[%d]) dmabuf : csc_convert() from gralloc to callback fail", __FUNCTION__, __LINE__);

                    csc_set_src_format(m_exynosThumbCSC,
                        ALIGN_DOWN(m_thumbnailW_temp, 16), ALIGN_DOWN(m_thumbnailH_temp,2),
                        0, 0, ALIGN(m_thumbnailW_temp, 16), ALIGN_DOWN(m_thumbnailH_temp,2),
                        V4L2_PIX_2_HAL_PIXEL_FORMAT(iTempColorformat), 1);
                    csc_set_dst_format(m_exynosThumbCSC,
                        m_thumbnailW, m_thumbnailH,
                        0, 0, m_thumbnailW, m_thumbnailH,
                ...
                if ((m_exynosThumbCSC && ALIGN_DOWN(iTempWidth, 16) == iTempWidth)
                    ...
                    csc_set_src_format(m_exynosThumbCSC,
                        ALIGN_DOWN(iTempWidth,16), ALIGN_DOWN(iTempHeight,2),
                        0, 0, ALIGN_DOWN(iTempWidth,16), ALIGN_DOWN(iTempHeight,2),
                        V4L2_PIX_2_HAL_PIXEL_FORMAT(iTempColorformat), 1);
                    csc_set_dst_format(m_exynosThumbCSC,
                        m_thumbnailW, m_thumbnailH,
                        0, 0, m_thumbnailW, m_thumbnailH,
                    ...
                    if (csc_convert(m_exynosThumbCSC) != 0)
                        ALOGE("ERR(%s[%d]) userptr : csc_convert() from gralloc to callback fail", __FUNCTION__, __LINE__);
                } else if ((m_exynosThumbCSC && ALIGN_DOWN(iTempWidth, 16) == iTempWidth)
                    && ((iTempWidth > (m_thumbnailW * 16)) || (iTempHeight > (m_thumbnailH * 16)))) {
                    int m_thumbnailW_temp = m_thumbnailW;
                    int m_thumbnailH_temp = m_thumbnailH;
                    ...
                    csc_set_src_buffer(m_exynosThumbCSC,
                        (void**)pcMainInputBuf, CSC_MEMORY_USERPTR);
                    csc_set_dst_buffer(m_exynosThumbCSC,
                        (void**)m_stThumbTempBuf.pcBuf, CSC_MEMORY_USERPTR);
                    if (csc_convert(m_exynosThumbCSC) != 0)
                        ALOGE("ERR(%s[%d]) userptr : csc_convert() from gralloc to callback fail", __FUNCTION__, __LINE__);

                    csc_set_src_format(m_exynosThumbCSC,
                        ALIGN_DOWN(m_thumbnailW_temp,16), ALIGN_DOWN(m_thumbnailH_temp,2),
                        0, 0, ALIGN_DOWN(m_thumbnailW_temp,16), ALIGN_DOWN(m_thumbnailH_temp,2),
                        V4L2_PIX_2_HAL_PIXEL_FORMAT(iTempColorformat), 1);
                    csc_set_dst_format(m_exynosThumbCSC,
                        m_thumbnailW, m_thumbnailH,
                        0, 0, m_thumbnailW, m_thumbnailH,
...
//            return ret;
        ...
        ALOGD("DEBUG(%s[%d]):JPEG thumbnail encoder start", __FUNCTION__, __LINE__);
...
int ExynosJpegEncoderForCamera::createIonClient(int ionClient)
{
    if (ionClient == 0) {
        ionClient = ion_open();
        if (ionClient < 0) {
            ALOGE("[%s]src ion_open failed, value = %d", __FUNCTION__, ionClient);
            return 0;
        }
    }
...
int ExynosJpegEncoderForCamera::allocJpegMemory(struct stJpegMem *pstMem, int iMemoryNum)
{
    ...
    for (i=0;i<iMemoryNum;i++) {
        ...
        ret = ion_alloc_fd(pstMem->ionClient, \
                                                        pstMem->iSize[i], 0, ION_HEAP_SYSTEM_MASK, 0, &pstMem->ionBuffer[i]);
        if (ret < 0) {
            ALOGE("[%s]ion_alloc_fd(%d) failed", __FUNCTION__, pstMem->iSize[i]);
