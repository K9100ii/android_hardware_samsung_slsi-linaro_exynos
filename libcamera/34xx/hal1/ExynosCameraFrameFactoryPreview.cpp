...
ExynosCameraFrame *ExynosCameraFrameFactoryPreview::createNewFrame(void)
{
    ...
    newEntity[INDEX(PIPE_GSC_CALLBACK)] = new ExynosCameraFrameEntity(PIPE_GSC_CALLBACK, ENTITY_TYPE_INPUT_OUTPUT, ENTITY_BUFFER_FIXED);
    frame->addSiblingEntity(NULL, newEntity[INDEX(PIPE_GSC_CALLBACK)]);
...
status_t ExynosCameraFrameFactoryPreview::stopPipes(void)
{
    ...
    if (m_pipes[INDEX(PIPE_GSC)] != NULL) {
        ret = m_pipes[INDEX(PIPE_GSC)]->stop();
        if (ret < 0) {
    ...
    if (m_pipes[INDEX(PIPE_GSC_CALLBACK)] != NULL) {
        ret = m_pipes[INDEX(PIPE_GSC_CALLBACK)]->stop();
        if (ret < 0) {
...
status_t ExynosCameraFrameFactoryPreview::m_setupConfig()
{
    ...
    nodeNums = m_nodeNums[INDEX(PIPE_GSC_CALLBACK)];
    nodeNums[OUTPUT_NODE] = PREVIEW_GSC_NODE_NUM;
    nodeNums[CAPTURE_NODE_1] = -1;
    nodeNums[CAPTURE_NODE_2] = -1;
