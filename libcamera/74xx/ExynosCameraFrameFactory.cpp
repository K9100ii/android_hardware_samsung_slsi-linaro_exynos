...
status_t ExynosCameraFrameFactory::setFlagFlipIgnore(__unused uint32_t pipeId, __unused bool flagFlipIgnore)
{
    CLOGE("ERR(%s[%d]):Must use the concreate class, don't use superclass", __FUNCTION__, __LINE__);
    return INVALID_OPERATION;
}
...
status_t ExynosCameraFrameFactory::stopThreadAndWait(uint32_t pipeId, int sleep, int times)
{
    ...
    if (status < 0) {
        CLOGE("ERR(%s[%d]):pipe(%d) stopThreadAndWait fail, ret(%d)", __FUNCTION__, __LINE__, pipeId, status);
        /* TODO: exception handling */
...
int ExynosCameraFrameFactory::getRunningFrameCount(uint32_t pipeId)
{
    ...
    runningFrameCount = m_pipes[pipeId]->m_getRunningFrameCount();
...
status_t ExynosCameraFrameFactory::m_initNodeInfo(void)
{
    CLOGD("DEBUG(%s[%d]): ", __FUNCTION__, __LINE__);
    return NO_ERROR;
}

status_t ExynosCameraFrameFactory::m_updateNodeInfo(void)
{
    CLOGD("DEBUG(%s[%d]): ", __FUNCTION__, __LINE__);
    return NO_ERROR;
}
