...
int ExynosJpegEncoderForCamera::encode(int *size, exif_attribute_t *exifInfo, char** pcJpegBuffer, debug_attribute_t *debugInfo)
{
    ...
    if (exifInfo != NULL) {
        ...
        if (exifLen <= (unsigned int) EXIF_LIMIT_SIZE || (debugInfo != NULL && debugInfo->num_of_appmarker
            && exifLen <= (unsigned int) EXIF_LIMIT_SIZE + totalDbgSize)) {
...
void ExynosJpegEncoderForCamera::unmapJpegMemory(__unused int *iFd, char **ppcBuf, int *piSize, int iMemoryNum)
{
    for (int i = 0; i < iMemoryNum; i++) {
