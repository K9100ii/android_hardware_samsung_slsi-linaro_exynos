...
#define MAX_SENSOR_VENDOR_CONFIG_SIZE	32
...
enum is_subscenario_id {
    ...
    ISS_SUB_SCENARIO_VIDEO_HIGHSPEED = 3,          // 3: video HD 100fps
...
enum is_subscenario_id {
    ...
    /* This is for just compile : START */
    //ISS_SUB_SCENARIO_STILL_PREVIEW = 0,     /* 0: still preview */
    //ISS_SUB_SCENARIO_VIDEO = 1,             /* 1: video */
...
struct sensor_vendorConfig {
	uint8_t		transmitAddrSize; // size of transmitData. 1 or 2
	uint16_t	transmitAddr;
	uint8_t		transmitDataSize; // size of transmitData. 1 or 2
...
struct camera2_sensor_ctl {
    ...
    uint8_t                     vendorConfigBurstSizeArray[MAX_SENSOR_VENDOR_CONFIG_SIZE];
    struct sensor_vendorConfig	vendorConfigData[MAX_SENSOR_VENDOR_CONFIG_SIZE];
};
...
enum aa_afmode_option_bit {
    ...
    AA_AFMODE_OPTION_BIT_SW_PDAF = 20,
    AA_AFMODE_OPTION_BIT_IMX230_PDAF = 21,
    AA_AFMODE_OPTION_BIT_IMX230_LASER_AF = 22,
};
...
struct camera2_aa_ctl {
    ...
    uint32_t					vendor_fastAfTargetPos;
    uint32_t					vendor_fastAfConfidence;
...
struct camera2_udm {
    ...
    struct camera2_laser_af_udm laserAf;
