...
public:
    ...
    status_t        checkVendorFeatures(const CameraParameters& params);
    status_t        checkVendorNightMode(const CameraParameters& params);
    status_t        checkVendorAntiShakeMode(const CameraParameters& params);
    status_t        checkVendorFlawLessMode(const CameraParameters& params);
    status_t        checkVendorHDRMode(const CameraParameters& params);
#endif
...
#ifdef CAMERA_VENDOR_TURNKEY_FEATURE
    void            m_setVendorMode(SFLIBRARY_MGR::SFLType mode, bool toggle);
#endif
...
public:
    ...
    /* Gets the current Manual focus distance. */
...
#ifdef CAMERA_VENDOR_TURNKEY_FEATURE
    status_t               setLibraryManager(sp<ExynosCameraSFLMgr> manager);
    sp<ExynosCameraSFLMgr> getLibraryManager();

    bool            getVendorMode(SFLIBRARY_MGR::SFLType mode);
#endif
 ...
 */
    ...
    void            setDebugInfoAttributeFromFrame(camera2_udm *udm);
...
public:
    ...
    status_t            setDNGCaptureInfo(struct camera2_shot_ext *shot);
#endif
