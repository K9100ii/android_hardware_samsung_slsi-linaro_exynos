...
ExynosCameraFrame * ExynosCamera3FrameReprocessingFactory::createNewFrame(uint32_t frameCount)
{
    ...
    if (ret != NO_ERROR)
        CLOGE("ERR(%s[%d]):frame(%d) metadata initialize fail", __FUNCTION__, __LINE__, m_frameCount);
...
status_t ExynosCamera3FrameReprocessingFactory::initPipes(void)
{
    ...
    CLOGI("INFO(%s[%d]): PictureSize(%dx%d)", __FUNCTION__, __LINE__, pictureW, pictureH);
    CLOGI("INFO(%s[%d]): MaxThumbnailSize(%dx%d)", __FUNCTION__, __LINE__, maxThumbnailW, maxThumbnailH);
    CLOGI("INFO(%s[%d]): PreviewBayerCropSize(%dx%d)", __FUNCTION__, __LINE__, bayerCropSize.w, bayerCropSize.h);
...
status_t ExynosCamera3FrameReprocessingFactory::startPipes(void)
{
    ...
    if (m_supportSingleChain == true) {
        ...
        if (ret != NO_ERROR) {
            CLOGE("ERR(%s[%d]):PIPE_GSC_REPROCESSING start fail, ret(%d)", __FUNCTION__, __LINE__, ret);
            /* TODO: exception handling */
            return INVALID_OPERATION;
        }
    }
...
status_t ExynosCamera3FrameReprocessingFactory::stopPipes(void)
{
    ...
    if (m_supportSingleChain == true) {
        ...
        if (m_pipes[INDEX(PIPE_GSC_REPROCESSING)]->isThreadRunning() == true) {
            ...
            if (ret != NO_ERROR) {
                CLOGE("ERR(%s[%d]):PIPE_GSC_REPROCESSING stopThread fail, ret(%d)", __FUNCTION__, __LINE__, ret);
                /* TODO: exception handling */
                return INVALID_OPERATION;
            }
        }
    ...
    if (m_supportSingleChain == true) {
        ...
        if (ret != NO_ERROR) {
            CLOGE("ERR(%s[%d]):PIPE_GSC_REPROCESSING stopThreadAndWait fail, ret(%d)", __FUNCTION__, __LINE__, ret);
        }
    }
