...
status_t ExynosCamera3Parameters::initYuvSizes() {
    ...
    for(int outputPort = 0; outputPort < getYuvStreamMaxNum(); outputPort++) {
        ALOGV("DEBUG(%s):Port[%d] Idx[%d], Size(%d x %d) / true", __FUNCTION__
...
status_t ExynosCamera3Parameters::getMinYuvSize(int* w, int* h) const {
    if(m_minYuvH == 0 || m_minYuvW == 0) {
        CLOGE("ERR(%s[%d]): Min YUV size is not initialized (w=%d, h=%d)"
            , __FUNCTION__, __LINE__, m_minYuvW, m_minYuvH);
        return INVALID_OPERATION;
    }
...
#endif
    {
        ...
        case CONFIG_MODE::HIGHSPEED_240:
            if (m_staticInfo->videoSizeLutHighSpeed240Max == 0
                ...
                ALOGE("ERR(%s[%d]):videoSizeLutHighSpeed240 is NULL", __FUNCTION__, __LINE__);
            } else {
...
status_t ExynosCamera3Parameters::checkPreviewFormat(const int streamPreviewFormat)
{
    ...
    if (curPreviewFormat != newPreviewFormat) {
        ...
        CLOGI("INFO(%s[%d]):curPreviewFormat %s newPreviewFormat %s",
                __FUNCTION__, __LINE__, curFormatName, newFormatName);
...
{
    CLOGV("INFO(%s):m_cameraInfo.hwPreviewFormat(%d)", __FUNCTION__, m_cameraInfo.hwPreviewFormat);
...
status_t ExynosCamera3Parameters::checkPictureSize(int pictureW, int pictureH)
{
    ...
    if (m_isSupportedPictureSize(pictureW, pictureH) == false) {
        ...
        CLOGE("ERR(%s):Invalid picture size(%dx%d)", __FUNCTION__, pictureW, pictureH);
...
//        m_params.setPictureSize(maxHwPictureW, maxHwPictureH);
    ...
    CLOGI("INFO(%s):newPicture Size (%dx%d), ratioId(%d)",
        "setParameters", pictureW, pictureH, m_cameraInfo.pictureSizeRatioId);
    ...
    if (curPictureW != pictureW || curPictureH != pictureH ||
        ...
        CLOGI("INFO(%s[%d]): Picture size changed: cur(%dx%d) -> new(%dx%d)",
                "setParameters", __LINE__, curPictureW, curPictureH, pictureW, pictureH);
...
enum companion_drc_mode ExynosCamera3Parameters::getRTDrc(void)
{
    ...
    CLOGV("WARN(%s[%d]):empty operation", __FUNCTION__, __LINE__);
#else
...
#ifdef USE_BINNING_MODE
    ALOGD("(%s)[%d] : getBinningMode(%d)",__func__, __LINE__, getBinningMode());
#endif
...
status_t ExynosCamera3Parameters::getPreviewBayerCropSize(ExynosRect *srcRect, ExynosRect *dstRect, bool applyZoom)
{
    ...
    if (getUsePureBayerReprocessing() == false
        ...
        ret = getCropRectAlign(dstRect->w, dstRect->h,
                ...
                0, 0.0f);
        if (ret != NO_ERROR) {
            CLOGE("ERR(%s[%d]):Failed to getCropRectAlign. previewBcrop %dx%d picture %dx%d",
                    __FUNCTION__, __LINE__,
        ...
        if (pictureCrop.w * SCALER_MAX_SCALE_UP_RATIO < pictureW
            ...
            CLOGW("WRN(%s[%d]):Zoom ratio is upto x%d pictureCrop %dx%d pictureTarget %dx%d",
                    __FUNCTION__, __LINE__,
    ...
    ALOGD("DEBUG(%s[%d]):hwBnsSize %dx%d, hwBcropSize %d,%d %dx%d",
            __FUNCTION__, __LINE__,
            srcRect->w, srcRect->h,
...
#if 1
    ALOGD("DEBUG(%s[%d]):maxSensorSize %dx%d, hwSensorSize %dx%d, previewSize %dx%d",
            __FUNCTION__, __LINE__,
            maxSensorW, maxSensorH, hwSensorW, hwSensorH, previewW, previewH);
    ALOGD("DEBUG(%s[%d]):hwPictureSize %dx%d, pictureSize %dx%d",
            __FUNCTION__, __LINE__,
            hwPictureW, hwPictureH, pictureW, pictureH);
    ALOGD("DEBUG(%s[%d]):pictureBayerCropSize %d,%d %dx%d",
            __FUNCTION__, __LINE__,
            cropX, cropY, cropW, cropH);
    ALOGD("DEBUG(%s[%d]):pictureFormat 0x%x, JPEG_INPUT_COLOR_FMT 0x%x",
            __FUNCTION__, __LINE__,
...
#ifdef DEBUG_PERFRAME
    ALOGD("DEBUG(%s[%d]):hwBdsSize (%dx%d)",
            __FUNCTION__, __LINE__, dstRect->w, dstRect->h);
#endif
...
status_t ExynosCamera3Parameters::getPreviewBdsSize(ExynosRect *dstRect, bool applyZoom)
{
    ...
    if (ret != NO_ERROR) {
        CLOGE("ERR(%s[%d]):Failed to m_getPreviewBdsSize()", __FUNCTION__, __LINE__);
        return ret;
    }
    ...
    if (ret != NO_ERROR)
        CLOGE("ERR(%s[%d]):Failed to getPreviewBayerCropSize()", __FUNCTION__, __LINE__);
    ...
    */
    for (int i = 0; i < getYuvStreamMaxNum(); i++) {
        ...
        if(yuvWidth > bdsSize.w || yuvHeight > bdsSize.h) {
            CLOGV("INFO(%s[%d]):Expanding BDS(%d x %d) size to BCrop(%d x %d) to handle YUV stream [%d, (%d x %d)]",
                    __FUNCTION__, __LINE__,
...
#ifdef DEBUG_PERFRAME
    CLOGD("DEBUG(%s[%d]):hwBdsSize %dx%d",
            __FUNCTION__, __LINE__, dstRect->w, dstRect->h);
#endif
...
status_t ExynosCamera3Parameters::getPictureBdsSize(ExynosRect *dstRect)
{
    ...
    if (ret != NO_ERROR)
        CLOGE("ERR(%s[%d]):Failed to getPictureBayerCropSize()",
                __FUNCTION__, __LINE__);
...
#ifdef DEBUG_PERFRAME
    CLOGD("DEBUG(%s[%d]):BDS %dx%d cropRegion %d,%d %dx%d",
            __FUNCTION__, __LINE__,
...
status_t ExynosCamera3Parameters::getPictureYuvCropSize(ExynosRect *yuvCropSize)
{
    ...
    if (isUseReprocessingIspInputCrop() == true
        ...
        ALOGE("ERR(%s[%d]):Picture YUV crop is NOT supported", __FUNCTION__, __LINE__);
...
{
    CLOGD("DEBUG(%s[%d]): m_flaguseDepthMap(%d)",  __FUNCTION__, __LINE__, m_flaguseDepthMap);
...
status_t ExynosCamera3Parameters::calcPictureBDSSize(ExynosRect *srcRect, ExynosRect *dstRect)
{
    ...
    ret = getPictureBayerCropSize(&bnsSize, &bayerCropSize);
    if (ret != NO_ERROR)
        CLOGE("ERR(%s[%d]):Failed to getPictureBayerCropSize()", __FUNCTION__, __LINE__);
...
#ifdef DEBUG_PERFRAME
    CLOGE("DEBUG(%s[%d]):hwBdsSize %dx%d Picture %dx%d",
            __FUNCTION__, __LINE__,
            dstRect->w, dstRect->h, pictureW, pictureH);
#endif
...
status_t ExynosCamera3Parameters::calcNormalToTpuSize(int srcW, int srcH, int *dstW, int *dstH)
{
    ...
    *dstW = disW;
    *dstH = disH;
    CLOGD("DEBUG(%s[%d]):HWVdis adjusted BDS Size (%d x %d) -> (%d x %d)",
        __FUNCTION__, __LINE__, srcW, srcH, disW, disH);
...
bool ExynosCamera3Parameters::getUseJpegCallbackForYuv(void)
{
    android_printAssert(NULL, LOG_TAG, "Not yet supported, assert!!!!");
