...
bool GrallocExynosCameraBufferManager::m_bufferCollectorThreadFunc(void)
{
    ...
    if (m_collectedBufferCount >= MAX_BUFFER_COLLECT_COUNT || m_dequeuedBufCount >= (m_allowedMaxBufCount - m_indexOffset - m_minUndequeuedBufCount) ) {
#ifdef EXYNOS_CAMERA_BUFFER_TRACE
        CLOGD("DEBUG(%s[%d]): bufferCollector just return. m_collectedBufferCount(%d) m_dequeuedBufCount(%d)", __FUNCTION__, __LINE__, m_collectedBufferCount, m_dequeuedBufCount);
#endif
        return false;
    }
    ...
    ret = m_allocator->dequeueBuffer(&bufHandle,
            ...
            &isLocked);
    if (ret != NO_ERROR) {
        CLOGE("ERR(%s[%d]):dequeueBuffer failed, dequeue(%d), collected(%d)", __FUNCTION__, __LINE__, m_dequeuedBufCount, m_collectedBufferCount);
        return false;
    }
