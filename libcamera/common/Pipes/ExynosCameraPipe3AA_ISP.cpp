...
status_t ExynosCameraPipe3AA_ISP::create(int32_t *sensorIds)
{
    ...
    ret = m_ispNode->create("ISP", m_cameraId);
    if (ret < 0) {
    ...
    ret = m_mainNode->create("3AA_output", m_cameraId);
    if (ret < 0) {
    ...
    ret = m_subNode->create("3AA_capture", m_cameraId, fd);
    if (ret < 0) {
...
status_t ExynosCameraPipe3AA_ISP::precreate(int32_t *sensorIds)
{
    ...
    ALOGI("INFO(%s[%d]):precreate() is succeed (%d) prepare", __FUNCTION__, __LINE__, getPipeId());
...
status_t ExynosCameraPipe3AA_ISP::postcreate(int32_t *sensorIds)
{
    ...
    m_prepareBufferCount = m_exynosconfig->current->pipeInfo.prepare[getPipeId()];
    ALOGI("INFO(%s[%d]):postcreate() is succeed (%d) prepare (%d)", __FUNCTION__, __LINE__, getPipeId(), m_prepareBufferCount);
...
status_t ExynosCameraPipe3AA_ISP::setupPipe(camera_pipe_info_t *pipeInfos)
{
    ...
    ret = m_ispNode->setControl(V4L2_CID_IS_SET_SETFILE, m_setfile);
    if (ret != NO_ERROR) {
...
status_t ExynosCameraPipe3AA_ISP::setMapBuffer(ExynosCameraBuffer *srcBuf, ExynosCameraBuffer *dstBuf)
{
    status_t ret = NO_ERROR;
    ret = m_setMapBuffer(m_mainNode, srcBuf);
    if( ret < 0 ) {
        CLOGE("ERR(%s[%d]): m_mainNode map buffer fail", __FUNCTION__, __LINE__);
    }

    ret = m_setMapBuffer(m_ispNode, dstBuf);
    if( ret < 0 ) {
        CLOGE("ERR(%s[%d]): m_ispNode map buffer fail", __FUNCTION__, __LINE__);
    }

    return ret;
}

status_t ExynosCameraPipe3AA_ISP::setMapBuffer(List<ExynosCameraBuffer>  *srcBufList, List<ExynosCameraBuffer>  *dstBufList)
{
    const int MAXSIZE = 2;
    status_t ret = NO_ERROR;
    List<ExynosCameraBuffer>::iterator r;
    ExynosCameraBuffer buffer;
    List<ExynosCameraBuffer>  *list[MAXSIZE] = {0};
    List<ExynosCameraBuffer>  *iter = NULL;
    ExynosCameraNode  *nodeList[MAXSIZE]= {0};
    ExynosCameraNode  *node = NULL;


    list[SRC_PIPE] = srcBufList;
    list[DST_PIPE] = dstBufList;
    nodeList[SRC_PIPE] = m_mainNode;
    nodeList[DST_PIPE] = m_ispNode;


    for(int i = 0; i < MAXSIZE ; i++ ) {
        iter = list[i];
        node = nodeList[i];
        if (iter == NULL || node == NULL || iter->empty()) {
            CLOGD("DEBUG(%s[%d]):list is NULL / empty or node is NULL ", __FUNCTION__, __LINE__);
            continue;
        }
        r = iter->begin()++;
        ...
        {
            buffer = *r;
            ret = m_setMapBuffer(node, &buffer);
            if( ret < 0 ) {
...
status_t ExynosCameraPipe3AA_ISP::m_putBuffer(void)
{
    ...
    if (m_runningFrameList[ispBuffer.index] != NULL) {
        ALOGE("ERR(%s):new buffer is invalid, we already get buffer index(%d), newFrame->frameCount(%d)",
            __FUNCTION__, ispBuffer.index, newFrame->getFrameCount());
        m_dumpRunningFrameList();
        return BAD_VALUE;
    }
    ...
    m_runningFrameList[ispBuffer.index] = newFrame;
    m_numOfRunningFrame++;
...
status_t ExynosCameraPipe3AA_ISP::m_getBuffer(void)
{
    ...
    if (ret != NO_ERROR || error != NO_ERROR) {
        ...
        if (fliteBuffer.addr[1] != NULL) {
            ret = m_updateMetadataToFrame(fliteBuffer.addr[1], ispBuffer.index);
            if (ret < 0) {
        ...
        /* complete frame */
        ret = m_completeFrame(&curFrame, ispBuffer, false);
        if (ret < 0) {
    ...
    ret = m_getFrameByIndex(&perframeFrame, ispBuffer.index);
    if (ret < 0) {
    ...
    m_activityControl->activityBeforeExecFunc(PIPE_POST_3AA_ISP, (void *)&ispBuffer);
...
status_t ExynosCameraPipe3AA_ISP::m_getIspBuffer(void)
{
    ...
    if (ret != NO_ERROR || index < 0) {
        ...
        if (shot_ext != NULL) {
            ALOGW("(%s[%d]):Shot done invalid, frame(cnt:%d, index(%d)) skip",
    ...
    /* check scp frame drop */
    if (shot_ext == NULL) {
        ALOGW("(%s[%d]):shot_ext is NULL", __FUNCTION__, __LINE__);
    } else if (shot_ext->node_group.capture[0].request == 0) {
        curFrame->setScpDrop(true);
    }
...
status_t ExynosCameraPipe3AA_ISP::dumpFimcIsInfo(bool bugOn)
{
    ...
    ret = m_ispNode->setControl(V4L2_CID_IS_DEBUG_DUMP, bugOn);
    if (ret != NO_ERROR)
        CLOGE("ERR(%s):m_ispNode->setControl failed", __FUNCTION__);
...
status_t ExynosCameraPipe3AA_ISP::syncLog(uint32_t syncId)
{
    ...
    ret = m_ispNode->setControl(V4L2_CID_IS_DEBUG_SYNC_LOG, syncId);
    if (ret != NO_ERROR)
        CLOGE("ERR(%s):m_ispNode->setControl failed", __FUNCTION__);
