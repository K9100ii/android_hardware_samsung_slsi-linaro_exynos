...
status_t ExynosCameraPipeISP::create(int32_t *sensorIds)
{
    ...
    ret = m_mainNode->create("ISP", m_cameraId);
    if (ret < 0) {
...
status_t ExynosCameraPipeISP::m_putBuffer(void)
{
    ...
    if (shot_ext != NULL) {
        ...
        ALOGE("ERR(%s[%d]): shot_ext is NULL. Skip to update Metadata. framecount(%d), buffer(%d)",
...
status_t ExynosCameraPipeISP::m_getBuffer(void)
{
    ...
    if (curFrame != NULL && shot_ext != NULL) {
        ...
        if (shot_ext->node_group.capture[1].request == 0)
            curFrame->setScpDrop(true);
    }
