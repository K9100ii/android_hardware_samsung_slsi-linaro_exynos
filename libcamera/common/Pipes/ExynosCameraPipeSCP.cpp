...
status_t ExynosCameraPipeSCP::create(int32_t *sensorIds)
{
    ...
    ret = m_mainNode->create("SCP", m_cameraId);
    if (ret < 0) {
...
retry:
    ...
    if (m_skipFrameQ.getSizeOfProcessQ()) {
        ALOGW("WARN(%s[%d]): DROP_SCP return skip buffer", __FUNCTION__, __LINE__);
        skipFrame = NULL;
        ...
        newFrame->getDstBuffer(getPipeId(), &newBuffer);
        skipFrame->setDstBuffer(getPipeId(), newBuffer);
        m_requestFrameQ.pushProcessQ(&newFrame);

        ret = skipFrame->setEntityState(getPipeId(), ENTITY_STATE_FRAME_DONE);
        if (ret < 0) {
            ALOGW("WRN(%s[%d]): setEntity state fail", __FUNCTION__, __LINE__);
        }

        ret = skipFrame->setDstBufferState(getPipeId(), ENTITY_BUFFER_STATE_ERROR);
        if (ret < 0) {
            ALOGE("ERR(%s): setDstBuffer state fail", __FUNCTION__);
            goto retry;
        }
    ...
    if (newBuffer.index < 0) {
        ...
        if (newFrame->getScpDrop()) {
            ALOGW("WARN(%s): DROP_SCP Queue to skipFrameQ", __FUNCTION__);
...
status_t ExynosCameraPipeSCP::m_getBuffer(void)
{
    ...
    do {
        ...
        if (ret == TIMED_OUT || curFrame == NULL) {
            ALOGW("WARN(%s[%d]): requestFrame is NULL", __FUNCTION__, __LINE__);
    ...
    curFrame->setDstBuffer(getPipeId(), curBuffer);
