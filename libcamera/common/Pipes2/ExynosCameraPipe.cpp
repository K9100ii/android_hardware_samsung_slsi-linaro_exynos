...
#if defined(USE_CAMERA2_API_SUPPORT) || defined(USE_HAL3_2_METADATA_INTERFACE)
...
status_t ExynosCameraPipe::setNodeInfos(__unused struct nodeInfos* nodeinfos, __unused bool flagReset)
{
    CLOGD("DEBUG(%s[%d]):do not support SUPPORT_GROUP_MIGRATION", __FUNCTION__, __LINE__);
    return NO_ERROR;
}

status_t ExynosCameraPipe::getNodeInfos(__unused struct nodeInfos* nodeinfos)
{
    CLOGD("DEBUG(%s[%d]):do not support SUPPORT_GROUP_MIGRATION", __FUNCTION__, __LINE__);
    return NO_ERROR;
}
...
status_t ExynosCameraPipe::setFlagFlipIgnore(bool flagFlipIgnore)
{
    CLOGD("DEBUG(%s[%d])", __FUNCTION__, __LINE__);
...
int ExynosCameraPipe::m_getRunningFrameCount(void)
{
