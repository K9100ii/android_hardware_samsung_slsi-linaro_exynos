...
int ExynosCameraActivitySpecialCapture::t_func3AAfter(void *args)
{
    ...
    if (shot_ext != NULL && m_specialCaptureMode == SCAPTURE_MODE_HDR) {
        ...
        if (m_OISCaptureFcount == 0
            ...
            ALOGD("DEBUG(%s[%d]):m_OISCaptureFcount (%d / %d) OISCaptureDelay(%d) firingStable(%d)",
                __FUNCTION__, __LINE__, m_OISCaptureFcount, shot_ext->shot.dm.request.frameCount, OISCaptureDelay
