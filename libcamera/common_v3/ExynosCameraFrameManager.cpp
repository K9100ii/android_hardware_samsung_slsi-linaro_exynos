...
{
    CLOGD(" Worker CREATE(name(%s) cameraid(%d) mode(%d) min_margin(%d) max_margin(%d)  ",
             name, cameraid, operMode, minMargin, maxMargin);
    m_init();
    m_setMargin(maxMargin, minMargin);
}
CreateWorker::~CreateWorker()
{
    CLOGD(" Worker DELETE(name(%s) cameraid(%d) mode(%d) ",
             m_name, m_cameraId, m_operMode);
    m_deinit();
}
...
ExynosCameraFrameSP_sptr_t CreateWorker::m_execute()
{
    ...
    case FRAMEMGR_OPER::SLIENT:
        ...
        if (frame == NULL) {
            CLOGE(" getframe failed, processQ size (%d)",
                 m_worklist->getSizeOfProcessQ());
            CLOGE(" Thread return (%d) Thread Run Status (%d)",
                 ret, m_thread->isRunning());
        }
...
{
    CLOGD(" Worker CREATE(name(%s) cameraid(%d) mode(%d) ",
             name, cameraid, operMode);
    m_operMode = operMode;
...
{
    CLOGD(" Worker CREATE(name(%s) cameraid(%d) mode(%d) ", name, cameraid, operMode);
...
RunWorker::~RunWorker()
{
    CLOGD(" Worker DELETE(name(%s) cameraid(%d) mode(%d) ", m_name, m_cameraId, m_operMode);
    m_deinit();
}
...
status_t RunWorker::stop()
{
    ...
    case FRAMEMGR_OPER::SLIENT:
        ...
        m_framekeyQueue->wakeupAll();
        m_thread->requestExitAndWait();
        CLOGD(" worker stopped remove ths remained frames(%d) queue(%d)", (int)m_worklist.size(), (int)m_framekeyQueue->getSizeOfProcessQ());
...
status_t RunWorker::dump()
{
    ...
    for (iter = m_worklist.begin() ; iter != m_worklist.end() ;) {
        ...
        if (detect == 0) {
            ...
            CLOGW(" delete memory leak detected FrameKey(%d) HAL-FrameCnt(%d) FrameType(%u)", detect->getUniqueKey(), detect->getFrameCount(), detect->getFrameType());
            detect.clear();
        }
                                  ...
                                  Mutex *lock)
{
    ...
    if (listRet.second == false) {
        ret = FRAMEMGR_ERRCODE::ERR;
        CLOGE(" insertFrame fail frame already exist!! HAL frameCnt(%d) UniqueKey (%u) ", frame->getFrameCount(), frame->getUniqueKey());
    }
    lock->unlock();
    return ret;
}
...
status_t RunWorker::m_dumpFrame()
{
    ...
    for (iter = m_worklist.begin() ; iter != m_worklist.end() ; ++iter) {
        ...
        if (item == 0) {
            ...
            CLOGW(" delete memory leak detected FrameKey(%d) HAL-FrameCnt(%d) FrameType(%u)", item->getUniqueKey(), item->getFrameCount(), item->getFrameType());
        }
                                                            ...
                                                            FRAMEMGR_OPER::MODE operMode)
{
    ...
    CLOGD(" FrameManager CREATE(name(%s) cameraid(%d) mode(%d)",
             name, cameraid, operMode);
...
{
    CLOGD(" FrameManager DELETE(name(%s) cameraid(%d) mode(%d)",
             m_name, m_cameraId, m_operMode);
    m_deinit();
}

ExynosCameraFrameSP_sptr_t ExynosCameraFrameManager::createFrame(ExynosCameraParameters *param, uint32_t framecnt, uint32_t frametype)
{
...
ExynosCameraFrameSP_sptr_t ExynosCameraFrameManager::m_createFrame(ExynosCameraParameters *param,
                                                                uint32_t framecnt,
                                                                uint32_t frametype)
{
    int ret = FRAMEMGR_ERRCODE::OK;
