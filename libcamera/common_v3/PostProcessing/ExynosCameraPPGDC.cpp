...
status_t ExynosCameraPPGDC::m_draw(ExynosCameraImage srcImage, ExynosCameraImage dstImage)
{
    status_t ret = NO_ERROR;

    android_printAssert(NULL, LOG_TAG, "ASSERT(%s[%d]):Invalid operation. \
                                        you must call m_draw(int numOfSrcImage, ExynosCameraImage *srcImage...) \
                                        , assert!!!!",
...
status_t ExynosCameraPPGDC::m_draw(int numOfSrcImage, ExynosCameraImage *srcImage,
                                   int numOfDstImage, ExynosCameraImage *dstImage)
{
    status_t ret = NO_ERROR;
    status_t funcRet = NO_ERROR;

    if (numOfSrcImage != m_srcImageCapacity.getNumOfImage()) {
        ret = INVALID_OPERATION;
        CLOGE("numOfSrcImage(%d) != m_srcImageCapacity.getNumOfImage()(%d). so, fail",
            numOfSrcImage, m_srcImageCapacity.getNumOfImage());
        goto done;
    }

    if (numOfDstImage != m_dstImageCapacity.getNumOfImage()) {
        ret = INVALID_OPERATION;
        CLOGE("numOfDstImage(%d) != m_dstImageCapacity.getNumOfImage()(%d). so, fail",
            numOfDstImage, m_dstImageCapacity.getNumOfImage());
        goto done;
    }
...
void ExynosCameraPPGDC::m_setName(void)
{
...
status_t ExynosCameraPPGDC::m_setFormat(ExynosCameraNode *node, ExynosCameraImage *image, enum IMAGE_POS nodeType)
{
    ...
    if (flagSetRequest == false) {
        CLOGV("Skip set pipeInfos nodeType(%d), setFormat(%d, %d), reqBuffers(%d)",
                (int)nodeType, rect.fullW, rect.fullH, 1);
        return ret;
    }

    if (rect.fullW == 0 || rect.fullH == 0) {
        CLOGW("Invalid size(%d x %d), skip setSize()",
            rect.fullW, rect.fullH);
        ret = INVALID_OPERATION;
        return ret;
    }

    ret = node->setSize(rect.fullW, rect.fullH);
    if (ret != NO_ERROR) {
        CLOGE("node->setSize(rect.fullW : %d, rect.fullH : %d) fail",
            rect.fullW,
            rect.fullH);
        return ret;
    }
    ...
    ret = node->setColorFormat(rect.colorFormat, image->buf.planeCount, 1);
    if (ret != NO_ERROR) {
