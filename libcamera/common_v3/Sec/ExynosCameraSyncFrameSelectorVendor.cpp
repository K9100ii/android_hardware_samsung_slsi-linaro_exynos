...
status_t ExynosCameraSyncFrameSelector::manageSyncFrameHoldList(ExynosCameraFrameSP_sptr_t frame, int pipeID, bool isSrc, int32_t dstPos)
{
    ...
    if (m_parameters->getDualCameraMode() == true) {
        /* just change the state in case of coming internal frame */
        ...
        ret = manageSyncFrameHoldList(frame, pipeID, isSrc, dstPos);
    }

    if (ret != NO_ERROR) {
        CLOGE("manageFrameHoldList fail!! (fcount(%d), frameType(%d), syncType(%d), " \
        ...
        int count, int pipeID, bool isSrc, int tryCount, int32_t dstPos)
{
    ...
    if (m_parameters->getDualCameraMode() == true) {
        /* start check */
        switch (m_parameters->getDualPostStandbyMode()) {
        case ExynosCameraParameters::DUAL_POSTSTANDBY_MODE_ACTIVE:
