...
int ExynosCameraActivitySpecialCapture::t_funcSCPAfter(void *args)
{
    ...
    CLOGV("(%d)(%d)(%d)", shot_stream->fvalid, shot_stream->fcount, m_hdrDropFcount[0]);
    CLOGV("");
    ...
    if (shot_stream != NULL && m_specialCaptureMode == SCAPTURE_MODE_HDR) {
        ...
        if ((m_hdrDropFcount[2] == shot_stream->fcount) ||
            ...
            CLOGV("drop fcount %d [%d %d %d][%d %d %d]", shot_stream->fcount,
...
int ExynosCameraActivitySpecialCapture::t_funcSCCBefore(__unused void *args)
{
    CLOGV("");
...
int ExynosCameraActivitySpecialCapture::t_funcSCCAfter(__unused void *args)
{
    CLOGV("");
