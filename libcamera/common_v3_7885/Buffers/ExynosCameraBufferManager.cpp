...
status_t ExynosCameraBufferManager::create(const char *name, int cameraId, void *defaultAllocator, bool isShared)
{
    ...
    if (isShared) {
        ...
        if (cameraId < 0) {
            CLOGW("can't not create(CAM%d)", cameraId);
            return INVALID_OPERATION;
        }

        for (int i = 0; i < CAMERA_ID_MAX; i++) {
            if (m_refFlag[i][BUFFER_MANAGER_API_CREATE] == true) {
                apiCalled = true;
                break;
            }
        }

        m_refFlag[cameraId][BUFFER_MANAGER_API_CREATE] = true;

        if (apiCalled) {
            CLOGI("create(CAM%d) will be skipped", cameraId);
            return ret;
        } else {
            m_cameraId = cameraId;
            strncpy(m_name, name, EXYNOS_CAMERA_NAME_STR_SIZE - 1);
            CLOGI("create(CAM%d) will be running", cameraId);
        }
...
status_t ExynosCameraBufferManager::create(const char *name, void *defaultAllocator, bool isShared)
{
    return create(name, 0, defaultAllocator, isShared);
}

void ExynosCameraBufferManager::init(int cameraId)
{
    ...
    if (m_isShared) {
        ...
        if (cameraId < 0) {
            CLOGW("can't not init(CAM%d)", cameraId);
            return;
        }

        for (int i = 0; i < CAMERA_ID_MAX; i++) {
            if (m_refFlag[i][BUFFER_MANAGER_API_INIT] == true) {
                apiCalled = true;
                break;
            }
        }

        m_refFlag[cameraId][BUFFER_MANAGER_API_INIT] = true;

        if (apiCalled) {
            CLOGI("init(CAM%d) will be skipped", cameraId);
            return;
        } else {
            CLOGI("init(CAM%d) will be running", cameraId);
        }
    }
#endif
    CLOGV("cameraId(%d", cameraId);
...
void ExynosCameraBufferManager::deinit(int cameraId)
{
    CLOGD("IN..cameraId(%d)", cameraId);
    ...
    if (m_isShared) {
        ...
        if (cameraId < 0) {
            CLOGW("can't deinit(CAM%d)", cameraId);
            m_clearRefFlag();
            return;
        }

        m_refFlag[cameraId][BUFFER_MANAGER_API_DEINIT] = false;
        m_refFlag[cameraId][BUFFER_MANAGER_API_CREATE] = false;
        m_refFlag[cameraId][BUFFER_MANAGER_API_INIT] = false;
        m_refFlag[cameraId][BUFFER_MANAGER_API_SET_INFO] = false;

        for (int i = 0; i < CAMERA_ID_MAX; i++) {
            if (m_refFlag[i][BUFFER_MANAGER_API_DEINIT] == true) {
                apiClear = false;
                break;
            }
        }
        ...
        if (apiClear) {
            CLOGI("deinit(CAM%d) will be running", cameraId);
            m_clearRefFlag();
        } else {
            CLOGI("deinit(CAM%d) will be skipped", cameraId);
            return;
        }
...
status_t ExynosCameraBufferManager::resetBuffers()
{
    /* same as deinit */
...
status_t ExynosCameraBufferManager::resetBuffers(int cameraId)
{
    CLOGV("IN... cameraId(%d)", cameraId);
    ...
    if (m_isShared) {
        ...
        if (cameraId < 0) {
            CLOGW("can't not resetBuffers(CAM%d)", cameraId);
            return INVALID_OPERATION;
        }

        m_refFlag[cameraId][BUFFER_MANAGER_API_RESET_BUFFERS] = false;

        for (int i = 0; i < CAMERA_ID_MAX; i++) {
            if (m_refFlag[i][BUFFER_MANAGER_API_RESET_BUFFERS] == true) {
                apiClear = false;
                break;
            }
        }
        ...
        if (apiClear) {
            CLOGI("resetBuffers(CAM%d) will be running", cameraId);
        } else {
            CLOGI("resetBuffers(CAM%d) will be skipped", cameraId);
            return ret;
        }
    }
#endif
...
status_t ExynosCameraBufferManager::alloc(int cameraId)
{
    EXYNOS_CAMERA_BUFFER_IN();
    ...
    if (m_isShared) {
        ...
        if (cameraId < 0) {
            CLOGW("can't not alloc(CAM%d)", cameraId);
            return INVALID_OPERATION;
        }

        for (int i = 0; i < CAMERA_ID_MAX; i++) {
            if (m_refFlag[i][BUFFER_MANAGER_API_ALLOC] == true) {
                apiCalled = true;
                break;
            }
        }

        m_refFlag[cameraId][BUFFER_MANAGER_API_ALLOC] = true;

        if (apiCalled) {
            CLOGI("alloc(CAM%d) will be skipped", cameraId);
            return ret;
        } else {
            CLOGI("alloc(CAM%d) will be running", cameraId);
        }
    }
#endif
    ...
    CLOGD("[cameraId(%d)] Allocate the buffer succeeded (allowed(%d), requested(%d), allocated(%d))",
        cameraId, m_allowedMaxBufCount, m_reqBufCount, m_allocatedBufCount);
    dumpBufferInfo();
...
#ifdef USE_DUAL_CAMERA
void ExynosCameraBufferManager::m_clearRefFlag(void)
{
    for (int i = 0; i < CAMERA_ID_MAX; i++)
        for (int j = 0; j < BUFFER_MANAGER_API_MAX; j++)
            m_refFlag[i][j] = false;
}
#endif
...
status_t ExynosCameraBufferManager::m_free(void)
{
    ...
    CLOGD("Free the buffer (m_flagAllocated=%d, m_allocatedBufCount=%d) --- dumpBufferInfo ---",
            m_flagAllocated, m_allocatedBufCount);
    dumpBufferInfo();
        ...
        int cameraId)
{
    ...
    if (m_isShared) {
        ...
        if (cameraId < 0) {
            CLOGW("can't not setInfo(CAM%d)", cameraId);
            return INVALID_OPERATION;
        }

        for (int i = 0; i < CAMERA_ID_MAX; i++) {
            if (m_refFlag[i][BUFFER_MANAGER_API_SET_INFO] == true)
                apiCalled = true;
        }

        m_refFlag[cameraId][BUFFER_MANAGER_API_SET_INFO] = true;
        m_refFlag[cameraId][BUFFER_MANAGER_API_RESET_BUFFERS] = true;
        m_refFlag[cameraId][BUFFER_MANAGER_API_DEINIT] = true;

        if (apiCalled) {
            CLOGI("setInfo(CAM%d) will be skipped", cameraId);
            return ret;
        } else {
            CLOGI("setInfo(CAM%d) will be running", cameraId);
        }
    }
#endif

    if (m_indexOffset > 0) {
        CLOGD("(CAM%d) buffer indexOffset(%d), Index[0 - %d] not used",
             cameraId, m_indexOffset, m_indexOffset);
    }
...
bool ExynosCameraBufferManager::isAvaliable(int bufIndex)
{
    ...
    if (bufIndex < 0 || bufIndex >= VIDEO_MAX_FRAME) {
        CLOGD("Invalid buffer index(%d)", bufIndex);
        return ret;
    }
...
#if defined(DEBUG_RAWDUMP) || defined(DEBUG_DUMP_IMAGE)
        mapNeeded = true;
#else
...
status_t ExynosCameraBufferManager::m_createDefaultAllocator(bool isCached)
{
    ...
    m_defaultAllocator = new ExynosCameraIonAllocator(m_cameraId);
...
void ExynosCameraBufferManager::dump(void)
{
    ...
    if (m_isShared) {
        for (int i = 0; i < CAMERA_ID_MAX; i++)
            for (int j = 0; j < BUFFER_MANAGER_API_MAX; j++)
                if (m_refFlag[i][j])
                    CLOGI("refFlag[CAM%d][API%d] = %d", i, j, m_refFlag[i][j]);
    }
#endif
...
#ifdef USE_DUAL_CAMERA
            CLOGI("[%s][m_buffer[%d].fd[%d]=%d] .addr=%p .size=%d .position=%d .permission=%d]",
                    m_isShared ? "Shared" : "Alone",
...
#ifndef USE_GRALLOC_BUFFER_COLLECTOR
func_exit:
#endif
    EXYNOS_CAMERA_BUFFER_OUT();
...
status_t ServiceExynosCameraBufferManager::m_release(int bufIndex)
{
    status_t ret = NO_ERROR;

    CLOGD("bufIndex(%d)", bufIndex);
