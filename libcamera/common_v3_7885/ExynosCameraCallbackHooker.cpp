                    ...
                    void* user)
{
    ...
    CLOGD2(CAM_ID "cameraId(%d)->setCallbacks(hooking callback)", m_cameraId);
                                    ...
                                    void *user)
{
    ...
    */
    ...
    if (flagCallCallback == true) {
        ...
        if (notifyCb) {
#ifdef USE_DUAL_CAMERA_INTERFACE_LOG_TRACE
            CLOGD2(CAM_ID "[NOTIFY] msg_type(0x%x), ext1(%d), ext2(%d), user(%p)",
...
#ifdef USE_DUAL_CAMERA_INTERFACE_LOG_TRACE
        CLOGD2(CAM_ID "[NOTIFY] skipped!! msg_type(0x%x), ext1(%d), ext2(%d), user(%p)",
                cameraId, "", msg_type, ext1, ext2, user);
#endif
    }
}
                                    ...
                                    void *user)
{
    ...
    if (callbackHooker->getHooking() == false) {
        ...
        if (dataCb) {
#ifdef USE_DUAL_CAMERA_INTERFACE_LOG_TRACE
            CLOGD2(CAM_ID "[CALLBACK] msg_type(0x%x), data(%p), index(%d), metadata(%p), user(%p)",
                    cameraId, "", msg_type, data, index, metadata, user);
#endif
            dataCb(msg_type, data, index, metadata, callbackHooker->m_getOriginCbUser());
        }
    } else {
        CLOGD2(CAM_ID "[CALLBACK] skipped!! msg_type(0x%x), data(%p), index(%d), metadata(%p), user(%p)",
            cameraId, "", msg_type, data, index, metadata, user);
                                    ...
                                    void *user)
{
    ...
    if (callbackHooker->getHooking() == false) {
        ...
        if (dataCbTimestamp) {
#ifdef USE_DUAL_CAMERA_INTERFACE_LOG_TRACE
            CLOGD2(CAM_ID "[CALLBACK_TMP] timestamp(%lld), msg_type(0x%x), data(%p), index(%d), user(%p)",
...
#ifdef USE_DUAL_CAMERA_INTERFACE_LOG_TRACE
        CLOGD2(CAM_ID "[CALLBACK_TMP] skipped!! timestamp(%lld), msg_type(0x%x), data(%p), index(%d), user(%p)",
            cameraId, "", timestamp, msg_type, data, index, user);
#endif
    }
}
