...
status_t ExynosCameraIonAllocator::init(bool isCached)
{
    ...
    if (m_ionClient == 0) {
        ...
        if (m_ionClient < 0) {
            ALOGE("ERR(%s):exynos_ion_open failed", __FUNCTION__);
            ret = BAD_VALUE;
            goto func_exit;
        }
    }
...
ExynosCameraMHBAllocator::ExynosCameraMHBAllocator(int cameraId)
{
    m_cameraId = cameraId;
    m_allocator = NULL;
}
...
status_t ExynosCameraGrallocAllocator::enqueueBuffer(buffer_handle_t *handle, Mutex *lock)
{
    ...
    TIME_LOGGER_UPDATE(m_cameraId, 0, PIPE_MCSC0, DURATION, PREVIEW_SERVICE_ENQUEUE, true);
#endif
    ret = m_allocator->enqueue_buffer(m_allocator, handle);
#ifdef TIME_LOGGER_PREVIEW_ENABLE
    TIME_LOGGER_UPDATE(m_cameraId, 0, PIPE_MCSC0, DURATION, PREVIEW_SERVICE_ENQUEUE, false);
#endif
    lock->lock();
