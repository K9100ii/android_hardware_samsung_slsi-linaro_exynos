...
public:
    ...
    virtual status_t getCaptureResult(camera3_capture_result_t *captureResult);
    virtual status_t getNofityMsg(camera3_notify_msg_t *notifyResult);
...
public:
    ...
    virtual status_t                       setSensorTimestamp(uint64_t timestamp) = 0;
    ...
    virtual status_t                       pushToResultList(ResultRequest result) = 0;
...
#ifdef USE_VRA_GROUP
    virtual void                           setDsInputPortId(int dsInputPortId) = 0;
    virtual int                            getDsInputPortId(void) = 0;
#endif
};
...
public:
    ...
    virtual status_t                       setSensorTimestamp(uint64_t timestamp);
    ...
    virtual status_t                       pushToResultList(ResultRequest result);
...
private:
    ...
    virtual status_t                       m_pushResult(ResultRequest result, ResultRequestMap *list, Mutex *lock);
...
private:
    ...
    uint64_t                      m_sensorTimeStamp;
    mutable Mutex                 m_sensorTimeStampLock;
...
public:
    ...
    ExynosCameraRequestSP_sprt_t           registerToServiceList(camera3_capture_request *srcRequest);
    ExynosCameraRequestSP_sprt_t           registerToServiceList(ExynosCameraRequestSP_sprt_t request);
    ExynosCameraRequestSP_sprt_t           registerToRunningList();
    status_t                       removeFromRunningList(uint32_t frameCount);
    ...
    status_t                       registerToResultList(ResultRequest result);
...
//    status_t                       m_getFactory(int key, ExynosCamera3FrameFactory** item, FrameFactoryMap *list, Mutex *lock);
    ...
    status_t                       m_callbackOpsCaptureResult(camera3_capture_result_t *result);
    status_t                       m_callbackOpsNotify(camera3_notify_msg_t *msg);
    ResultRequest                  m_callbackOutputBuffers(ExynosCameraRequestSP_sprt_t request, ResultRequest result);
    ...
    bool                           m_streamCallbackThreadFunc(int id, bool testflag = false);
#if 0
    /* Other helper functions */
...
private:
    ...
    bool                          m_yuvCallbackStallThreadFunc0(void);
    bool                          m_yuvCallbackStallThreadFunc1(void);
    bool                          m_yuvCallbackStallThreadFunc2(void);
    bool                          m_yuvCallbackStallThreadFunc3(void);
    bool                          m_yuvCallbackStallThreadFunc4(void);
