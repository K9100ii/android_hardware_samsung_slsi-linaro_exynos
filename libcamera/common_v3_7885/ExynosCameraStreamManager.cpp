...
void ExynosCameraStreamManager::m_init()
{
    ...
    for (int i = 0; i < ExynosCamera3Parameters::YUV_OUTPUT_PORT_ID_MAX; i++)
...
{
    if (outputPortId < 0 || outputPortId >= ExynosCamera3Parameters::YUV_OUTPUT_PORT_ID_MAX) {
        ALOGE("ERR(%s[%d]):Invalid outputPortId %d",
...
int ExynosCameraStreamManager::getOutputPortId(int streamId)
{
    ...
    case HAL_STREAM_ID_CALLBACK:
        startIndex = ExynosCamera3Parameters::YUV_0;
        endIndex = ExynosCamera3Parameters::YUV_MAX;
        break;
    case HAL_STREAM_ID_CALLBACK_STALL:
        startIndex = ExynosCamera3Parameters::YUV_STALL_0;
        endIndex = ExynosCamera3Parameters::YUV_STALL_MAX;
        break;
...
status_t ExynosCameraStreamManager::m_increaseYuvStreamCount(int streamId)
{
    ...
    switch (streamId % HAL_STREAM_ID_MAX) {
        case HAL_STREAM_ID_CALLBACK:
            /* Reprocessing stream */
            yuvIndex = ExynosCamera3Parameters::YUV_STALL_0
                        + getTotalYuvStreamCount();
            m_setYuvStreamId(yuvIndex, streamId);
        case HAL_STREAM_ID_PREVIEW:
        case HAL_STREAM_ID_VIDEO:
            /* Preview stream */
            yuvIndex = ExynosCamera3Parameters::YUV_0
                        + getTotalYuvStreamCount();
            m_setYuvStreamId(yuvIndex, streamId);
            m_yuvStreamCount++;
            break;
