...
status_t ExynosCameraMCPipe::flushInputFrameQ(void)
{
    int size = m_inputFrameQ->getSizeOfProcessQ();
...
status_t ExynosCameraMCPipe::flushRequestFrameQ(void)
{
    int size = m_requestFrameQ->getSizeOfProcessQ();

    CLOGD(" IN(%d)", size);

    while (size--) {
        m_getBufferThreadFunc();
        if (m_flagTryStop == true)
            break;
    }
...
status_t ExynosCameraMCPipe::resetInputs()
{
    return m_resetInputs();
}
...
#ifdef DEBUG_DUMP_IMAGE
    m_dumpFrameQ = new frame_queue_t(m_dumpBufferThread);
#endif
...
#ifdef DEBUG_DUMP_IMAGE
    m_dumpFrameQ->setWaitTime(5500000000);      /* 5.5 sec */
#endif
...
bool ExynosCameraMCPipe::m_dumpBufferThreadFunc(void)
{
    ...
    ret = m_dumpFrameQ->waitAndPopProcessQ(&newFrame);
    if (ret == TIMED_OUT) {
        return ret;
    ...
    if (newFrame != NULL) {
        uint32_t frameCount = newFrame->getFrameCount();

        if (m_dumpBufferCount >= DEBUG_DUMP_MAX ||
        ...
        for (int i = OUTPUT_NODE; i < MAX_OUTPUT_NODE; i++) {
            ...
            if (m_node[i] != NULL && DEBUG_DUMP_CONDITION(pipeId)) {
                ...
                if (ret != NO_ERROR) {
                    ...
                    directDumpToFile(&buffer, m_cameraId, frameCount);
                }
            }
        }
...
p_err:
    ...
    return m_checkThreadLoop(m_dumpFrameQ);
}
#endif
...
#ifdef TIME_LOGGER_QBUF_ENABLE
    TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId(), INTERVAL, QBUF, true);
#endif
...
#ifdef TIME_LOGGER_QBUF_ENABLE
                TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId((enum NODE_TYPE)i), DURATION, QBUF, true);
#endif
                ret = m_node[i]->putBuffer(&(buffer[i]));
#ifdef TIME_LOGGER_QBUF_ENABLE
                TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId((enum NODE_TYPE)i), DURATION, QBUF, false);
#endif
                if (ret != NO_ERROR) {
...
#ifdef TIME_LOGGER_DQBUF_ENABLE
    TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId(), INTERVAL, DQBUF, true);
#endif
...
#ifdef TIME_LOGGER_DQBUF_ENABLE
        TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId((enum NODE_TYPE)i), DURATION, DQBUF, true);
#endif
        ret = m_node[i]->getBuffer(&(buffer[i]), &(bufferIndex[i]));
#ifdef TIME_LOGGER_DQBUF_ENABLE
        TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId((enum NODE_TYPE)i), DURATION, DQBUF, false);
#endif
        nodeDqRet[i] = ret;
        if (ret != NO_ERROR) {
...
//#endif
    ...
    if (captureNodeCount == 0 || checkRet != NO_ERROR) {
        ...
        m_getInternalFrameLogCnt = 0;
#ifdef DEBUG_DUMP_IMAGE
        m_dumpFrameQ->pushProcessQ(&newFrame);
#endif
    }
...
status_t  ExynosCameraMCPipe::m_resetInputs()
{
    CLOGD("m_resetInputs reset setInput");
