...
status_t ExynosCameraPipeJpeg::m_run(void)
{
    ...
    if (m_parameters->getDualCameraMode() == true &&
            ...
            CLOGD("re-set for dual camera, picture size(%dx%d), thumbnail size(%dx%d)",
                    pictureRect.w, pictureRect.h, thumbnailRect.w, thumbnailRect.h);
    }
#endif
...
#ifdef SAMSUNG_DUAL_CAPTURE_SOLUTION
    if (m_parameters->getDualCameraReprocessingSyncType() == SYNC_TYPE_SYNC
        && m_parameters->getFusionCaptureMode(getCameraId(), false)) {
        memset(m_shot_ext2, 0x00, sizeof(struct camera2_shot_ext));
        newFrame->getDynamicMeta(m_shot_ext2, OUTPUT_NODE_2);
        newFrame->getUserDynamicMeta(m_shot_ext2, OUTPUT_NODE_2);
        m_parameters->setExifChangedAttribute(&exifInfo, &pictureRect, &thumbnailRect, &m_shot_ext->shot, false, &m_shot_ext2->shot);
    } else {
...
jpeg_encode_done:
    if (ret != NO_ERROR) {
        ...
        CLOGD("[pictureW %d][pictureH %d][pictureFormat %c%c%c%c]",
            pictureRect.w, pictureRect.h,
            v4l2Format2Char(pictureRect.colorFormat, 0),
            v4l2Format2Char(pictureRect.colorFormat, 1),
            v4l2Format2Char(pictureRect.colorFormat, 2),
            v4l2Format2Char(pictureRect.colorFormat, 3));
    }
...
#ifdef SAMSUNG_DUAL_CAPTURE_SOLUTION
    m_shot_ext2 = new struct camera2_shot_ext;
#endif
}
