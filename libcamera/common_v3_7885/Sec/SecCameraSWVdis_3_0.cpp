...
status_t m_swVDIS_init()
{
    ...
    if (m_swVDIS_InW == 4032 && m_swVDIS_InH == 2268) { //UHD Case
        m_swVDIS_Delay = 15;
    }
    else {
        m_swVDIS_Delay = 21;
    ...
    if (m_swVDIS_UHD) {
        VDIS_LOG("VDIS_HAL_INIT: VS_INIT_DONE: Input: %d x %d  Output: %d x %d Delay: %d OutDelay: %d NUM_InBuffer: %d NUM_OutBuffer: %d",
            m_swVDIS_InW, m_swVDIS_InH, m_swVDIS_OutW, m_swVDIS_OutH, m_swVDIS_Delay, m_swVDIS_OutDelay, NUM_VDIS_UHD_BUFFERS, m_sw_VDIS_OutBufferNum);
    } else {
        VDIS_LOG("VDIS_HAL_INIT: VS_INIT_DONE: Input: %d x %d  Output: %d x %d Delay: %d OutDelay: %d NUM_InBuffer: %d NUM_OutBuffer: %d",
                          ...
                          float swVDIS_ExposureValue,  int oisGain, swVDIS_FaceData_t *FaceData, int swVDIS_ExposureTime)
{
    ...
    if(FaceData->FaceNum > NUM_OF_DETECTED_FACES) {
        FaceData->FaceNum = NUM_OF_DETECTED_FACES;
    }
    ...
    m_swVDIS_pluginData.Timestamp = swVDIS_timeStamp;
    m_swVDIS_pluginData.TimestampBoot = swVDIS_timeStampBoot;
...
int m_swVDIS_GetVDISOutInfo(nsecs_t *timeStamp)
{
    int outIndex = -1;
...
status_t m_swVDIS_GetPreviewVDISInfo()
{
    if (m_VDISpluginHandle == NULL) {
        ALOGD("Plugin Handle is NULL");
        return -1;
    ...
    m_swVDIS_Preview.prev_left = m_swVDIS_Offset[store_cnt].left;
    m_swVDIS_Preview.prev_top = m_swVDIS_Offset[store_cnt].top;
