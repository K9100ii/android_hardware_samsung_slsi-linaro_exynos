...
status_t ExynosCameraFrameJpegReprocessingFactory::m_constructPipes(void)
{
    ...
    /* LLS PlugIn */
    pipeId = PIPE_LLS_REPROCESSING;
    m_pipes[INDEX(pipeId)] = (ExynosCameraPipe*)new ExynosCameraPipePlugIn(m_cameraId, m_parameters, m_flagReprocessing, m_nodeNums[INDEX(PIPE_LLS_REPROCESSING)]);
...
int ExynosCameraFrameJpegReprocessingFactory::getScenario(int pipeId)
{
    int scenario = 0;
    return scenario;
}
