...
/* #define LOG_NDEBUG 0 */
#define LOG_TAG "ExynosCameraFrameJpegReprocessingFactorySec"
#include <log/log.h>
...
ExynosCameraFrameSP_sptr_t ExynosCameraFrameJpegReprocessingFactory::createNewFrame(uint32_t frameCount, bool useJpegFlag)
{
    ...
    if (m_supportPureBayerReprocessing == false
        ...
        if (m_parameters->getLLSCaptureOn() == true) {
            parentPipeId = PIPE_LLS_REPROCESSING;
...
status_t ExynosCameraFrameJpegReprocessingFactory::m_setupConfig(void)
{
    CLOGI("");
    ...
    case 5:
        nodeMcscpJpeg = FIMC_IS_VIDEO_M3P_NUM;
        nodeMcscpThumb = FIMC_IS_VIDEO_M4P_NUM;
        nodeMcscpDs = FIMC_IS_VIDEO_M5P_NUM;
        break;
    case 3:
        nodeMcscpJpeg = FIMC_IS_VIDEO_M1P_NUM;
        nodeMcscpThumb = FIMC_IS_VIDEO_M2P_NUM;
    ...
    m_deviceInfo[pipeId].nodeNum[nodeType] = nodeMcscpJpeg;
    m_deviceInfo[pipeId].bufferManagerType[nodeType] = BUFFER_MANAGER_ION_TYPE;
    ...
    m_deviceInfo[pipeId].nodeNum[nodeType] = nodeMcscpThumb;
    m_deviceInfo[pipeId].bufferManagerType[nodeType] = BUFFER_MANAGER_ION_TYPE;
...
#ifdef USE_LLS_REPROCESSING
    m_nodeNums[INDEX(PIPE_LLS_REPROCESSING)][OUTPUT_NODE] = 0;
    m_nodeNums[INDEX(PIPE_LLS_REPROCESSING)][CAPTURE_NODE_1] = -1;
    m_nodeNums[INDEX(PIPE_LLS_REPROCESSING)][CAPTURE_NODE_2] = -1;
#endif

    return NO_ERROR;
}

status_t ExynosCameraFrameJpegReprocessingFactory::m_constructPipes(void)
{
    CLOGI("");
    ...
    /* LLS PlugIn */
    pipeId = PIPE_LLS_REPROCESSING;
    m_pipes[INDEX(pipeId)] = (ExynosCameraPipe*)new ExynosCameraPipePlugIn(m_cameraId, m_configurations, m_parameters, m_flagReprocessing, m_nodeNums[INDEX(PIPE_LLS_REPROCESSING)]);
    m_pipes[INDEX(pipeId)]->setPipeId(pipeId);
    m_pipes[INDEX(pipeId)]->setPipeName("PIPE_LLS_REPROCESSING");
#endif

    /* 3AA for Reprocessing */
    pipeId = PIPE_3AA_REPROCESSING;
    m_pipes[INDEX(pipeId)] = (ExynosCameraPipe*)new ExynosCameraMCPipe(m_cameraId, m_configurations, m_parameters, m_flagReprocessing, &m_deviceInfo[INDEX(pipeId)]);
    m_pipes[INDEX(pipeId)]->setPipeId(pipeId);
    ...
    /* VRA */
    pipeId = PIPE_VRA_REPROCESSING;
    m_pipes[INDEX(pipeId)] = (ExynosCameraPipe*)new ExynosCameraPipeVRA(m_cameraId, m_configurations, m_parameters, m_flagReprocessing, m_deviceInfo[INDEX(pipeId)].nodeNum);
    m_pipes[INDEX(pipeId)]->setPipeId(pipeId);
    m_pipes[INDEX(pipeId)]->setPipeName("PIPE_VRA_REPROCESSING");

    /* GSC for Reprocessing */
    pipeId = PIPE_GSC_REPROCESSING;
    m_pipes[INDEX(pipeId)] = (ExynosCameraPipe*)new ExynosCameraPipeGSC(m_cameraId, m_configurations, m_parameters, m_flagReprocessing, m_nodeNums[INDEX(pipeId)]);
    m_pipes[INDEX(pipeId)]->setPipeId(pipeId);
    ...
    if (m_flagHWFCEnabled == false || (m_flagHWFCEnabled && m_parameters->isHWFCOnDemand())) {
        /* JPEG for Reprocessing */
        pipeId = PIPE_JPEG_REPROCESSING;
        m_pipes[INDEX(pipeId)] = (ExynosCameraPipe*)new ExynosCameraPipeJpeg(m_cameraId, m_configurations, m_parameters, m_flagReprocessing, m_nodeNums[INDEX(pipeId)]);
        m_pipes[INDEX(pipeId)]->setPipeId(pipeId);
...
status_t ExynosCameraFrameJpegReprocessingFactory::m_initFrameMetadata(ExynosCameraFrameSP_sptr_t frame)
{
    status_t ret = NO_ERROR;
