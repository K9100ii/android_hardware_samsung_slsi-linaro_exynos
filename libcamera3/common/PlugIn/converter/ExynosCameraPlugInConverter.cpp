...
status_t ExynosCameraPlugInConverter::m_convertBufferInfoFromFrame(ExynosCameraFrameSP_sptr_t frame, Map_t *map)
{
    ...
    for (i = 0; i < m_curSrcBufCnt; i++) {
        ...
        CLOGI("[%d] source frame(%d) buf[%d, %p, %dbyte [%d, %d, %d, %d, %d, %d]",
                i,
                frame->getFrameCount(),
                m_srcBufPlaneCnt[i],
                m_srcBuf[i][0],
    ...
    (*map)[PLUGIN_SRC_BUF_CNT]         = (Map_data_t)srcBufCount;
    ...
    for (i = 0; i < m_curDstBufCnt; i++) {
        ...
        CLOGI("[%d] dst frame(%d) buf[%d, %p, %dbyte [%d, %d, %d, %d, %d, %d]",
                i,
                frame->getFrameCount(),
                m_dstBufPlaneCnt[i],
                m_dstBuf[i][0],
    ...
    (*map)[PLUGIN_DST_BUF_CNT]         = (Map_data_t)dstBufCount;
