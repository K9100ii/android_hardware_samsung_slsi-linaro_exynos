...
status_t FakeFusion::execute(Map_t *map)
{
    ...
    for (int i = 0; i < mapSrcBufCnt; i++) {
        PLUGIN_LOGD("src[%d/%d]::(adr:%p, P%d, S%d, [%d, %d, %d, %d, %d, %d] format :%d",
                i,
                mapSrcBufCnt,
                mapSrcBufAddr[i][0],
                mapSrcBufPlaneCnt[i],
    ...
    PLUGIN_LOGD("dst[%d]::(adr:%p, P%d, S%d, [%d, %d, %d, %d, %d, %d] format :%d",
            mapDstBufCnt,
            mapDstBufAddr[0],
            mapDstBufPlaneCnt[0],
            mapDstBufSize[0][0],
    ...
    PLUGIN_LOGD("copyRatio:%f", copyRatio);
    ...
    for (int i = 0; i < mapSrcBufCnt; i++) {
        ...
        case 2:
            ...
            break;
        default:
            PLUGIN_ASSERT("Invalid planeCount(%d), assert!!!!", planeCount);
            break;
        }

        PLUGIN_LOGD("fusion emulationn running ~~~ memcpy(%p, %p, %d)" \
