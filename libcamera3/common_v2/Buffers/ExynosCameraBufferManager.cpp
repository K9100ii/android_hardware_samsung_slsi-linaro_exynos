                                                             ...
                                                             const ExynosCameraBuffer* buffer)
{
    ...
    int imagePlaneCount = (m_hasMetaPlane == true) ? buffer->planeCount - 1 : buffer->planeCount;
    ...
    case 3:
        planeIndex -= 1;
        if ((unsigned int)privateHandle->size2 < buffer->size[planeIndex]) {
            isInvalidBuffer |= true;
            CLOGE("[B%d P%d]fd %d size %d < %d",
                    buffer->index, planeIndex,
                    privateHandle->fd2,
                    privateHandle->size2, buffer->size[planeIndex]);
        }
    case 2:
        planeIndex -= 1;
        if ((unsigned int)privateHandle->size1 < buffer->size[planeIndex]) {
            isInvalidBuffer |= true;
            CLOGE("[B%d P%d]fd %d size %d < %d",
                    buffer->index, planeIndex,
                    privateHandle->fd1,
                    privateHandle->size1, buffer->size[planeIndex]);
        }
    case 1:
    default:
        planeIndex -= 1;
        if ((unsigned int)privateHandle->size < buffer->size[planeIndex]) {
            isInvalidBuffer |= true;
            CLOGE("[B%d P%d]fd %d size %d < %d",
                    buffer->index, planeIndex,
                    privateHandle->fd,
                    privateHandle->size, buffer->size[planeIndex]);
        }
        break;
    }
    ...
    if (isInvalidBuffer == true) {
        CLOGE("[B%d]format %X imageSize %dx%d usage %ju|%ju ionFlags %X",
                buffer->index,
                privateHandle->format,
                privateHandle->width, privateHandle->height,
                privateHandle->producer_usage, privateHandle->consumer_usage,
                privateHandle->flags);
...
SWExynosCameraBufferManager::SWExynosCameraBufferManager()
{
    ExynosCameraBufferManager::init();

    m_allocationThread = new allocThread(this, &SWExynosCameraBufferManager::m_allocationThreadFunc, "allocationThreadFunc");

    for (int bufIndex = 0; bufIndex < SWBUFFER_MAX_COUNT; bufIndex++) {
        for (int planeIndex = 0; planeIndex < EXYNOS_CAMERA_BUFFER_MAX_PLANES; planeIndex++) {
            m_swBuffer[bufIndex].fd[planeIndex] = -1;
        }
    }
}
...
status_t SWExynosCameraBufferManager::resetBuffers(void)
{
    /* same as deinit */
    /* clear buffers except releasing the memory */
    status_t ret = NO_ERROR;
...
status_t SWExynosCameraBufferManager::alloc(void)
{
    EXYNOS_CAMERA_BUFFER_IN();
    ExynosCameraAutoTimer autoTimer(__FUNCTION__);
...
status_t SWExynosCameraBufferManager::setInfo(buffer_manager_configuration_t info)
{
    EXYNOS_CAMERA_BUFFER_IN();
    Mutex::Autolock lock(m_lock);
    ...
    if (info.reqBufCount < 0 || SWBUFFER_MAX_COUNT < info.reqBufCount) {
        CLOGE("abnormal value [reqBufCount=%d]", info.reqBufCount);
        ret = BAD_VALUE;
        goto func_exit;
    }
    ...
    for (int bufIndex = m_indexOffset; bufIndex < info.allowedMaxBufCount + m_indexOffset; bufIndex++) {
        for (int planeIndex = 0; planeIndex < info.planeCount; planeIndex++) {
            ...
            m_swBuffer[bufIndex].size[curPlaneIndex]         = info.size[planeIndex];
            m_swBuffer[bufIndex].bytesPerLine[curPlaneIndex] = info.bytesPerLine[planeIndex];
        }
        ...
        m_swBuffer[bufIndex].planeCount = info.planeCount;
        m_swBuffer[bufIndex].type       = info.type;
        m_swBuffer[bufIndex].batchSize  = info.batchSize;
    }
...
void SWExynosCameraBufferManager::m_resetSequenceQ()
{
    Mutex::Autolock lock(m_availableBufferIndexQLock);
    m_availableBufferIndexQ.clear();

    for (int bufIndex = m_indexOffset; bufIndex < m_allocatedBufCount + m_indexOffset; bufIndex++)
        m_availableBufferIndexQ.push_back(m_swBuffer[bufIndex].index);

    return;
}

status_t SWExynosCameraBufferManager::m_setAllocator(void *allocator)
{
    return m_setDefaultAllocator(allocator);
}

/* User Process need to check the index of buffer returned from "getBuffer()" */
status_t SWExynosCameraBufferManager::getBuffer(
        int  *reqBufIndex,
        enum EXYNOS_CAMERA_BUFFER_POSITION position,
        struct ExynosCameraBuffer *buffer)
{
...
reDo:
    ...
    m_swBuffer[bufferIndex].index = bufferIndex;
    *reqBufIndex = bufferIndex;
    *buffer      = m_swBuffer[bufferIndex];
...
status_t SWExynosCameraBufferManager::putBuffer(
        int bufIndex,
        enum EXYNOS_CAMERA_BUFFER_POSITION position)
{
    EXYNOS_CAMERA_BUFFER_IN();
...
status_t SWExynosCameraBufferManager::updateStatus(
        int bufIndex,
        int driverValue,
        enum EXYNOS_CAMERA_BUFFER_POSITION   position,
        enum EXYNOS_CAMERA_BUFFER_PERMISSION permission)
{
    ...
    m_swBuffer[bufIndex].index = bufIndex;
    m_swBuffer[bufIndex].status.driverReturnValue = driverValue;
    m_swBuffer[bufIndex].status.position          = position;
    m_swBuffer[bufIndex].status.permission        = permission;

    return NO_ERROR;
}

status_t SWExynosCameraBufferManager::getStatus(
        int bufIndex,
        struct ExynosCameraBufferStatus *bufStatus)
{
    *bufStatus = m_swBuffer[bufIndex].status;

    return NO_ERROR;
}

status_t SWExynosCameraBufferManager::getIndexByFd(int fd, int *index)
{
    if (fd < 0) {
        CLOGE("Invalid FD %d", fd);
        return BAD_VALUE;
    ...
    *index = -1;
    for (int bufIndex = m_indexOffset; bufIndex < m_reqBufCount + m_indexOffset; bufIndex++) {
        if (m_swBuffer[bufIndex].fd[0] == fd) {
            *index = bufIndex;
            break;
        }
    }
...
bool SWExynosCameraBufferManager::isAvaliable(int bufIndex)
{
    bool ret = false;

    switch (m_swBuffer[bufIndex].status.permission) {
    case EXYNOS_CAMERA_BUFFER_PERMISSION_NONE:
    case EXYNOS_CAMERA_BUFFER_PERMISSION_AVAILABLE:
        ret = true;
        break;
...
status_t SWExynosCameraBufferManager::m_defaultAlloc(int bIndex, int eIndex, bool isMetaPlane)
{
    EXYNOS_CAMERA_BUFFER_IN();
    ...
    for (bufIndex = bIndex; bufIndex < eIndex; bufIndex++) {
        if (isMetaPlane == true) {
            planeIndexStart = m_swBuffer[bufIndex].getMetaPlaneIndex();
            planeIndexEnd   = planeIndexStart + 1;
            mask  = EXYNOS_CAMERA_BUFFER_ION_MASK_NONCACHED;
            flags = EXYNOS_CAMERA_BUFFER_ION_FLAG_NONCACHED;
            estimatedBase = EXYNOS_CAMERA_BUFFER_ION_WARNING_TIME_NONCACHED;
            ...
            planeIndexStart = 0;
            planeIndexEnd   = m_swBuffer[bufIndex].getMetaPlaneIndex();
            switch (m_swBuffer[bufIndex].type) {
            case EXYNOS_CAMERA_BUFFER_ION_NONCACHED_TYPE:
                mask  = EXYNOS_CAMERA_BUFFER_ION_MASK_NONCACHED;
                flags = EXYNOS_CAMERA_BUFFER_ION_FLAG_NONCACHED;
                estimatedBase = EXYNOS_CAMERA_BUFFER_ION_WARNING_TIME_NONCACHED;
            ...
            case EXYNOS_CAMERA_BUFFER_INVALID_TYPE:
            default:
                CLOGE("buffer type is invaild (%d)", (int)m_swBuffer[bufIndex].type);
                break;
            }
        }
        ...
        for (int planeIndex = planeIndexStart; planeIndex < planeIndexEnd; planeIndex++) {
            if (m_swBuffer[bufIndex].fd[planeIndex] >= 0) {
                CLOGE("buffer[%d].fd[%d] = %d already allocated",
                         bufIndex, planeIndex, m_swBuffer[bufIndex].fd[planeIndex]);
                continue;
            }

            if (m_defaultAllocator->alloc(
                    m_swBuffer[bufIndex].size[planeIndex],
                    &(m_swBuffer[bufIndex].fd[planeIndex]),
                    &(m_swBuffer[bufIndex].addr[planeIndex]),
                    mask,
                    flags,
                    mapNeeded) != NO_ERROR) {
#if defined(RESERVED_MEMORY_ENABLE) && defined(RESERVED_MEMORY_REALLOC_WITH_ION)
                if (m_swBuffer[bufIndex].type == EXYNOS_CAMERA_BUFFER_ION_RESERVED_TYPE) {
                    CLOGE("Realloc with ion:bufIndex(%d), m_reservedMemoryCount(%d),"
                        " non-cached. so, alloc ion memory instead of reserved memory",
                        bufIndex, m_reservedMemoryCount);
                    ...
                    flags = EXYNOS_CAMERA_BUFFER_ION_FLAG_NONCACHED;
                    estimatedBase = EXYNOS_CAMERA_BUFFER_ION_WARNING_TIME_NONCACHED;
                } else if (m_swBuffer[bufIndex].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_RESERVED_TYPE) {
                    CLOGE("Realloc with ion:bufIndex(%d), m_reservedMemoryCount(%d),"
                        " cached. so, alloc ion memory instead of reserved memory",
                        bufIndex, m_reservedMemoryCount);
                    ...
                    CLOGE("m_defaultAllocator->alloc(bufIndex=%d, planeIndex=%d, size=%d) failed",
                        bufIndex, planeIndex, m_swBuffer[bufIndex].size[planeIndex]);
...
#endif
            if (isMetaPlane == false) {
                bufferSize = bufferSize + m_swBuffer[bufIndex].size[planeIndex];
            }
        }
        ...
        if (isMetaPlane == false) {
            ...
            durationTimeSum += durationTime;
            CLOGD("duration time(%5d msec):(type=%d, bufIndex=%d, size=%.2f, batchSize=%d)",
                 (int)durationTime, m_swBuffer[bufIndex].type, bufIndex,
                 (float)bufferSize / (float)(1024 * 1024), m_swBuffer[bufIndex].batchSize);

            estimatedTime = estimatedBase * bufferSize / EXYNOS_CAMERA_BUFFER_1MB;
            if (estimatedTime < durationTime) {
                CLOGW("estimated time(%5d msec):(type=%d, bufIndex=%d, size=%d)",
                     (int)estimatedTime, m_swBuffer[bufIndex].type, bufIndex, (int)bufferSize);
            }
        }
    ...
    if ((planeIndexEnd - planeIndexStart) == 1) {
        ...
        CLOGD("Duration time of buffer(Plane:%d~%d) allocation(%5d msec)",
                planeIndexStart, (planeIndexEnd - 1), (int)durationTimeSum);
    }
...
status_t SWExynosCameraBufferManager::m_defaultFree(int bIndex, int eIndex, bool isMetaPlane)
{
    EXYNOS_CAMERA_BUFFER_IN();
...
bool SWExynosCameraBufferManager::m_checkInfoForAlloc(void)
{
    EXYNOS_CAMERA_BUFFER_IN();

    bool ret = true;

    if (m_reqBufCount < 0 || SWBUFFER_MAX_COUNT < m_reqBufCount) {
        CLOGE("buffer Count in out of bound [m_reqBufCount=%d]", m_reqBufCount);
        ret = false;
        goto func_exit;
    }
    ...
    for (int bufIndex = m_indexOffset; bufIndex < m_reqBufCount + m_indexOffset; bufIndex++) {
        if (m_swBuffer[bufIndex].planeCount < 0
         || VIDEO_MAX_PLANES <= m_swBuffer[bufIndex].planeCount) {
            CLOGE("plane Count in out of bound [m_buffer[bIndex].planeCount=%d]",
                 m_swBuffer[bufIndex].planeCount);
            ret = false;
            goto func_exit;
        }

        for (int planeIndex = 0; planeIndex < m_swBuffer[bufIndex].planeCount; planeIndex++) {
            int curPlaneIndex = ((m_hasMetaPlane == true && planeIndex == m_swBuffer[bufIndex].planeCount -1) ?
                                m_swBuffer[bufIndex].getMetaPlaneIndex() : planeIndex);

            if (m_swBuffer[bufIndex].size[curPlaneIndex] == 0) {
                CLOGE("size is empty [m_buffer[%d].size[%d]=%d]",
                    bufIndex, curPlaneIndex, m_swBuffer[bufIndex].size[curPlaneIndex]);
                ret = false;
                goto func_exit;
            }
        }
...
int SWExynosCameraBufferManager::getNumOfAvailableBuffer(void)
{
    int numAvailable = 0;

    for (int i = m_indexOffset; i < m_allocatedBufCount + m_indexOffset; i++) {
        if (m_swBuffer[i].status.permission == EXYNOS_CAMERA_BUFFER_PERMISSION_AVAILABLE)
            numAvailable++;
    }
...
int SWExynosCameraBufferManager::getNumOfAvailableAndNoneBuffer(void)
{
    int numAvailable = 0;

    for (int i = m_indexOffset; i < m_allocatedBufCount + m_indexOffset; i++) {
        if (m_swBuffer[i].status.permission == EXYNOS_CAMERA_BUFFER_PERMISSION_AVAILABLE ||
            m_swBuffer[i].status.permission == EXYNOS_CAMERA_BUFFER_PERMISSION_NONE)
            numAvailable++;
    }
...
void SWExynosCameraBufferManager::printBufferState(void)
{
    for (int i = m_indexOffset; i < m_allocatedBufCount + m_indexOffset; i++) {
        CLOGV("m_buffer[%d].fd[0]=%d, position=%d, permission=%d]",
            i, m_swBuffer[i].fd[0],
            m_swBuffer[i].status.position, m_swBuffer[i].status.permission);
    }

    return;
}

void SWExynosCameraBufferManager::printBufferState(int bufIndex, int planeIndex)
{
    CLOGI("m_buffer[%d].fd[%d]=%d, .status.permission=%d]",
        bufIndex, planeIndex, m_swBuffer[bufIndex].fd[planeIndex],
        m_swBuffer[bufIndex].status.permission);

    return;
}

void SWExynosCameraBufferManager::printBufferInfo(
        __unused const char *funcName,
        __unused const int lineNum,
        int bufIndex,
        int planeIndex)
...
void SWExynosCameraBufferManager::dumpBufferInfo(void)
{
#ifdef EXYNOS_CAMERA_DUMP_BUFFER_INFO
    for (int bufIndex = m_indexOffset; bufIndex < m_allocatedBufCount + m_indexOffset; bufIndex++) {
        int totalPlaneCount = m_getTotalPlaneCount(m_swBuffer[bufIndex].planeCount,
                                                    m_swBuffer[bufIndex].batchSize, m_hasMetaPlane);
        for (int planeIndex = 0; planeIndex < totalPlaneCount; planeIndex++) {
            CLOGI("[m_buffer[%d].fd[%d]=%d] .addr=%p .size=%d .position=%d .permission=%d]",
                    m_swBuffer[bufIndex].index, planeIndex,
                    m_swBuffer[bufIndex].fd[planeIndex],
                    m_swBuffer[bufIndex].addr[planeIndex],
                    m_swBuffer[bufIndex].size[planeIndex],
                    m_swBuffer[bufIndex].status.position,
                    m_swBuffer[bufIndex].status.permission);
        }
    }
    printBufferQState();
...
status_t SWExynosCameraBufferManager::m_alloc(int bIndex, int eIndex)
{
    status_t ret = NO_ERROR;
...
status_t SWExynosCameraBufferManager::m_free(int bIndex, int eIndex)
{
    status_t ret = NO_ERROR;
...
status_t SWExynosCameraBufferManager::m_constructBufferContainer(int bIndex, int eIndex)
{
    EXYNOS_CAMERA_BUFFER_IN();
    ...
    for (int bufIndex = bIndex; bufIndex < eIndex; bufIndex++) {
        /* Get buffer container FD for same size planes */
        int imagePlaneCount = (m_hasMetaPlane == true) ?
                            (m_swBuffer[bufIndex].planeCount - 1)
                            : m_swBuffer[bufIndex].planeCount;
        ...
        for (int planeIndex = 0; planeIndex < imagePlaneCount; planeIndex++) {
            /* Gather single FDs */
            int fds[EXYNOS_CAMERA_BUFFER_MAX_PLANES];
            for (int batchIndex = 0; batchIndex < m_swBuffer[bufIndex].batchSize; batchIndex++) {
                fds[batchIndex] = m_swBuffer[bufIndex].fd[(batchIndex * imagePlaneCount) + planeIndex];
            }

            ret = m_defaultAllocator->createBufferContainer(fds,
                                                            m_swBuffer[bufIndex].batchSize,
                                                            &m_swBuffer[bufIndex].containerFd[planeIndex]);
            if (ret != NO_ERROR) {
                CLOGE("[B%d P%d]Failed to createBufferContainer. batchSize %d ret %d",
                        bufIndex, planeIndex, m_swBuffer[bufIndex].batchSize, ret);
                ret = INVALID_OPERATION;
                goto func_exit;
            }
            ...
            for (int batchIndex = 0; batchIndex < m_swBuffer[bufIndex].batchSize; batchIndex++) {
                int curPlaneIndex = (batchIndex * imagePlaneCount) + planeIndex;
                ret = m_defaultAllocator->free(m_swBuffer[bufIndex].size[curPlaneIndex],
                                            &(m_swBuffer[bufIndex].fd[curPlaneIndex]),
                                            &(m_swBuffer[bufIndex].addr[curPlaneIndex]),
                                            m_flagNeedMmap);
                if (ret != NO_ERROR) {
                    CLOGE("[B%d P%d FD%d]Failed to free. ret %d",
                            bufIndex, planeIndex, m_swBuffer[bufIndex].fd[curPlaneIndex], ret);
                    /* continue */
                }
            }
        }
        ...
        timer.stop();
        CLOGD("duration time(%5d msec):(bufIndex=%d, batchSize=%d, fd=%d/%d/%d)",
                (int)timer.durationMsecs(), bufIndex, m_swBuffer[bufIndex].batchSize,
                m_swBuffer[bufIndex].containerFd[0], m_swBuffer[bufIndex].containerFd[1],
                m_swBuffer[bufIndex].containerFd[2]);
    }
...
status_t SWExynosCameraBufferManager::m_destructBufferContainer(int bIndex, int eIndex)
{
    EXYNOS_CAMERA_BUFFER_IN();
    ...
    for (int bufIndex = bIndex; bufIndex < eIndex; bufIndex++) {
        ...
        for (int planeIndex = 0; planeIndex < imagePlaneCount; planeIndex++) {
            ret = m_defaultAllocator->free(m_swBuffer[bufIndex].size[planeIndex],
                                           &(m_swBuffer[bufIndex].containerFd[planeIndex]),
                                           &dummyAddr, /*mapNeeded */false);
            if (ret != NO_ERROR) {
                CLOGE("[B%d P%d FD%d]Failed to free containerFd. ret %d",
                        bufIndex, planeIndex, m_swBuffer[bufIndex].containerFd[planeIndex], ret);
                /* continue */
            }
        }
    }
...
status_t SWExynosCameraBufferManager::m_increase(int increaseCount)
{
    CLOGD("IN..");
    ExynosCameraAutoTimer autoTimer(__FUNCTION__);
    ...
    /* set the buffer information */
    for (int bufIndex = m_allocatedBufCount + m_indexOffset; bufIndex < m_allocatedBufCount + m_indexOffset + increaseCount; bufIndex++) {
        for (int planeIndex = 0; planeIndex < m_swBuffer[0].planeCount; planeIndex++) {
            if (m_swBuffer[0].size[planeIndex] == 0) {
                CLOGE("abnormal value [size=%d]",
                     m_swBuffer[0].size[planeIndex]);
                ret = BAD_VALUE;
                goto func_exit;
            }
            m_swBuffer[bufIndex].size[planeIndex]         = m_swBuffer[0].size[planeIndex];
            m_swBuffer[bufIndex].bytesPerLine[planeIndex] = m_swBuffer[0].bytesPerLine[planeIndex];
        }
        m_swBuffer[bufIndex].planeCount = m_swBuffer[0].planeCount;
        m_swBuffer[bufIndex].type       = m_swBuffer[0].type;
    }
    ...
    if (m_hasMetaPlane == true) {
        if (m_defaultAlloc(m_allocatedBufCount + m_indexOffset,
                            m_allocatedBufCount + m_indexOffset + increaseCount,
                            m_hasMetaPlane) != NO_ERROR) {
            CLOGE("m_defaultAlloc failed");
            ret = INVALID_OPERATION;
            goto func_exit;
        }
...
status_t SWExynosCameraBufferManager::m_decrease(void)
{
    CLOGD("IN..");
    ExynosCameraAutoTimer autoTimer(__FUNCTION__);
    ...
    if (m_hasMetaPlane == true) {
        if (m_defaultFree(bufferIndex-1 + m_indexOffset,
                            bufferIndex + m_indexOffset,
                            m_hasMetaPlane) != NO_ERROR) {
            CLOGE("m_defaultFree failed");
            ret = INVALID_OPERATION;
            goto func_exit;
...
status_t SWExynosCameraBufferManager::increase(int increaseCount)
{
    ExynosCameraAutoTimer autoTimer(__FUNCTION__);
    Mutex::Autolock lock(m_lock);
    status_t ret = NO_ERROR;
...
bool SWExynosCameraBufferManager::m_allocationThreadFunc(void)
{
    status_t ret = NO_ERROR;
    int increaseCount = 1;
    ...
    for (int count = 0; count < increaseCount; count++) {
        ...
        if (ret < 0) {
            ...
            m_lock.lock();
            m_availableBufferIndexQ.push_back(m_swBuffer[m_allocatedBufCount + m_indexOffset].index);
            m_allocatedBufCount++;
            m_lock.unlock();
        }
