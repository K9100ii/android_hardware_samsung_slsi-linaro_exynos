...
status_t ExynosCameraMCPipe::m_putBuffer(void)
{
    ...
    if (ret == TIMED_OUT) {
#ifdef USE_DUAL_CAMERA
        if ((m_cameraId == CAMERA_ID_BACK && dualOperationMode == DUAL_OPERATION_MODE_SLAVE) ||
            (m_cameraId == CAMERA_ID_BACK_1 && dualOperationMode == DUAL_OPERATION_MODE_MASTER)) {
            CLOGV("not pair. cameraId(%d), dualOperationMode(%d)", m_cameraId, dualOperationMode);
            return NO_ERROR;
        } else
#endif
        {
    ...
    TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId(), INTERVAL, QBUF, 0);
...
status_t ExynosCameraMCPipe::m_getBuffer(void)
{
    ...
    for (int i = OUTPUT_NODE; i < MAX_OUTPUT_NODE; i++) {
        ...
        TIME_LOGGER_UPDATE(m_cameraId, newFrame->getFrameCount(), getPipeId(), INTERVAL, DQBUF, 0);
