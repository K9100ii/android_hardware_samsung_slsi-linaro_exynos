...
status_t ExynosCameraPipeSTK_PREVIEW::start(void)
{
    ...
    if (bSTKInit == false) {
        m_parameters->getSize(HW_INFO_HW_PREVIEW_SIZE, &previewRect.w, &previewRect.h);
...
status_t ExynosCameraPipeSTK_PREVIEW::m_destroy(void)
{
...
status_t ExynosCameraPipeSTK_PREVIEW::m_run(void)
{
    ...
    seriesShotMode = m_configurations->getModeValue(CONFIGURATION_SERIES_SHOT_MODE);
