...
/*#define LOG_NDEBUG 0 */
#define LOG_TAG "ExynosCameraPPUniPluginBeauty_Capture"

#include "ExynosCameraPPUniPluginBeauty_Capture.h"

ExynosCameraPPUniPluginBeauty_Capture::~ExynosCameraPPUniPluginBeauty_Capture()
{
}

status_t ExynosCameraPPUniPluginBeauty_Capture::m_draw(ExynosCameraImage *srcImage,
                                           ...
                                           ExynosCameraParameters *params)
{
    ...
    case BEAUTY_CAPTURE_IDLE:
        break;
    case BEAUTY_CAPTURE_RUN:
        m_timer.start();
        ...
        break;
    case BEAUTY_CAPTURE_DEINIT:
        CLOGD("[BEAUTY] BEAUTY_CAPTURE_DEINIT");
        break;
    default:
        break;
    }
...
status_t ExynosCameraPPUniPluginBeauty_Capture::m_UniPluginInit(void)
{
    status_t ret = NO_ERROR;
...
void ExynosCameraPPUniPluginBeauty_Capture::m_init(void)
{
    CLOGD(" ");
    ...
    m_beautyStatus = BEAUTY_CAPTURE_IDLE;

    m_refCount = 0;
}

status_t ExynosCameraPPUniPluginBeauty_Capture::start(void)
{
    status_t ret = NO_ERROR;
    ...
    m_beautyStatus = BEAUTY_CAPTURE_RUN;

    return ret;
}

status_t ExynosCameraPPUniPluginBeauty_Capture::stop(bool suspendFlag)
{
    status_t ret = NO_ERROR;
    ...
    m_beautyStatus = BEAUTY_CAPTURE_DEINIT;

    return ret;
}

status_t ExynosCameraPPUniPluginBeauty_Capture::process(ExynosCameraImage *srcImage,
                                                            ExynosCameraImage *dstImage,
                                                            ExynosCameraParameters *params)
{
    int w = 0 ,  h = 0;
    ...
    bufferInfo.inBuffU = (char *)srcImage[0].buf.addr[0] + (srcImage[0].rect.w * srcImage[0].rect.h);
    ...
    bufferInfo.outBuffU = (char *)dstImage[0].buf.addr[0] + (dstImage[0].rect.w * dstImage[0].rect.h);
...
#endif
    ALOGV("[BEAUTY_CAPTURE] bokehRecordingHint %d", bokehRecordingHint);

    ret = m_UniPluginSet(UNI_PLUGIN_INDEX_BEAUTY_MODE, &bokehRecordingHint);
    if (ret < 0) {
        CLOGE("[BEAUTY_CAPTURE] Plugin set(UNI_PLUGIN_INDEX_BEAUTY_MODE) failed!!");
    }

    ret = m_UniPluginSet(UNI_PLUGIN_INDEX_BUFFER_INFO, &bufferInfo);
    if (ret < 0) {
        CLOGE("[BEAUTY_CAPTURE] Plugin set(UNI_PLUGIN_INDEX_BUFFER_INFO) failed!!");
    }
    ...
    CLOGV("[BEAUTY_CAPTURE] w:%d, h:%d Retouch(%d)", w, h, beautyFaceRetouchLevel);

    ret = m_UniPluginSet(UNI_PLUGIN_INDEX_BEAUTY_LEVEL_SOFTEN, &beautyFaceRetouchLevel);
    if (ret < 0) {
        CLOGE("[BEAUTY_CAPTURE] Plugin set(UNI_PLUGIN_INDEX_BEAUTY_LEVEL_SOFTEN) failed!!");
    }

    ret = m_UniPluginProcess();
    if (ret < 0) {
        ALOGE("[BEAUTY_CAPTURE] BEAUTY plugin process failed!!");
