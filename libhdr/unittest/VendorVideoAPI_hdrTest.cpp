    ...
    void                 *src)
{
    ...
    for (i = 0; i < pHdr10PlusInfo->data.num_windows; i++) {
        ...
        if (pHdr10PlusInfo->data.tone_mapping.tone_mapping_flag[i]) {
            ...
            if (pHdr10PlusInfo->data.tone_mapping.num_bezier_curve_anchors[i] > 9) {
                ALOGW("[%s] num_bezier_curve_anchors[%d]: (%d) is invalid", __FUNCTION__, i, pHdr10PlusInfo->data.tone_mapping.num_bezier_curve_anchors[i]);
                return -1;
            }
    ...
    for (i = 0; i < windows; i++) {
        ...
        if (pHdr10PlusInfo->data.tone_mapping.tone_mapping_flag) {
            ...
            if (pHdr10PlusInfo->data.tone_mapping.num_bezier_curve_anchors > 9) {
                ALOGW("[%s] num_bezier_curve_anchors(%d) is invalid", __FUNCTION__, pHdr10PlusInfo->data.tone_mapping.num_bezier_curve_anchors);
                return -1;
            }
