...
int ADec_SetParams(u32 ulHandle, SEIREN_PARAMCMD paramCmd, unsigned long long pulValues)
{
    u32 cmd = paramCmd << 16;
...
int ADec_GetParams(u32 ulHandle, SEIREN_PARAMCMD paramCmd, unsigned long long *pulValues)
{
    u32 cmd = paramCmd << 16;

    cmd |= SEIREN_IOCTL_CH_GET_PARAMS;
    ioctl(ulHandle, cmd, pulValues);
    ALOGD("%s: val:%llu. handle:%d", __func__, *pulValues, ulHandle);
