...
static uchar max_op(uchar a, uchar b) 
{   
	return a > b ? a : b;
}

#ifdef ARM_MALI //mali OpenCL
__kernel void vx_dilate3x3(int aw, int ah, int asx, int asy, __global uchar *a, 
                     int bw, int bh, int bsx, int bsy, __global uchar *b)
#else
__kernel void vx_dilate3x3(int aw, int ah, int asx, int asy, __global void *a, 
...
#endif                     
{
 ...
 //   printf("dilate : (%d,%d) src(%dx%d)->dst(%dx%d) \n",x,y,aw,ah,bw,bh);
   ...
   if (y == 0 || x == 0 || x == (w - 1) || y == (h - 1))
    ...
    out = pixels[0];
    for (i = 1; i < 9; i++) {
    	out = max_op(out, pixels[i]); //dilate
    	}
