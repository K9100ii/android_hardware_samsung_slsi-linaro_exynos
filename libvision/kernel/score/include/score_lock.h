...
void score_bakery_lock(int id);

/// @brief  Unlock by bakery algorithm.
/// @param  id Device id like SCORE, CPU or IVA.
void score_bakery_unlock(int id);

#ifdef CONFIG_BAKERY_LOCK
#define SCORE_BAKERY_LOCK(id) score_bakery_lock(id)
#else
#define SCORE_BAKERY_LOCK(id) NULL
#endif

#ifdef CONFIG_BAKERY_UNLOCK
#define SCORE_BAKERY_UNLOCK(id) score_bakery_unlock(id)
#else
