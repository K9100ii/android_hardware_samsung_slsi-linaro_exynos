...
#ifndef EXYNOS_VPU_KERNEL_HARRIS_H
#define EXYNOS_VPU_KERNEL_HARRIS_H
...
class ExynosVpuKernelHarris : public ExynosVpuKernel {
private:
...
public:
    /* Contructor */
    ExynosVpuKernelHarris(vx_char *name, vx_uint32 param_num);

    /* Destructor */
    virtual ~ExynosVpuKernelHarris();
...
class ExynosVpuTaskIfWrapperHarris : public ExynosVpuTaskIfWrapper {
private:
    vx_status postProcessTask(const vx_reference parameters[]);

public:
    ExynosVpuTaskIfWrapperHarris(ExynosVpuKernelHarris *kernel, vx_uint32 task_index);
};
