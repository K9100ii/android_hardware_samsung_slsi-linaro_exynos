...
#ifndef EXYNOS_VPU_KERNEL_LUT_H
#define EXYNOS_VPU_KERNEL_LUT_H
...
class ExynosVpuKernelLut : public ExynosVpuKernel {
private:
...
public:
    /* Contructor */
    ExynosVpuKernelLut(vx_char *name, vx_uint32 param_num);

    /* Destructor */
    virtual ~ExynosVpuKernelLut();
...
class ExynosVpuTaskIfWrapperLut : public ExynosVpuTaskIfWrapper {
private:
    vx_status preProcessTask(const vx_reference parameters[]);

public:
    ExynosVpuTaskIfWrapperLut(ExynosVpuKernelLut *kernel, vx_uint32 task_index);
};
