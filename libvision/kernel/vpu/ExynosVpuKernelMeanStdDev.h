...
#ifndef EXYNOS_VPU_KERNEL_MEANSTDDEV_H
#define EXYNOS_VPU_KERNEL_MEANSTDDEV_H
...
class ExynosVpuKernelMeanStdDev : public ExynosVpuKernel {
private:
...
public:
    /* Contructor */
    ExynosVpuKernelMeanStdDev(vx_char *name, vx_uint32 param_num);

    /* Destructor */
    virtual ~ExynosVpuKernelMeanStdDev();
...
class ExynosVpuTaskIfWrapperMeanStdDev : public ExynosVpuTaskIfWrapper {
private:
    vx_status postProcessTask(const vx_reference parameters[]);

public:
    ExynosVpuTaskIfWrapperMeanStdDev(ExynosVpuKernel *kernel, vx_uint32 task_index);
};
