...
#define LOG_TAG "ExynosVpuKernelPhase"
#include <cutils/log.h>

#include "ExynosVpuKernelPhase.h"

#include "vpu_kernel_util.h"

#include "td-binary-phase.h"
...
static vx_uint16 td_binary[] =
    TASK_test_binary_phase_from_VDE_DS;

vx_status
ExynosVpuKernelPhase::inputValidator(vx_node node, vx_uint32 index)
{
...
vx_status
ExynosVpuKernelPhase::outputValidator(vx_node node, vx_uint32 index, vx_meta_format meta)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
...
ExynosVpuKernelPhase::ExynosVpuKernelPhase(vx_char *name, vx_uint32 param_num)
                                       : ExynosVpuKernel(name, param_num)
{
    strcpy(m_task_name, "phase");
}

ExynosVpuKernelPhase::~ExynosVpuKernelPhase(void)
{
...
vx_status
ExynosVpuKernelPhase::setupBaseVxInfo(const vx_reference parameters[])
{
...
vx_status
ExynosVpuKernelPhase::initTask(vx_node node, const vx_reference *parameters)
{
    vx_status status;
...
vx_status
ExynosVpuKernelPhase::initTaskFromBinary(void)
{
...
ExynosVpuTaskIf*
ExynosVpuKernelPhase::initTask0FromBinary(void)
{
    vx_status status = VX_SUCCESS;
    int ret = NO_ERROR;
...
vx_status
ExynosVpuKernelPhase::initTaskFromApi(void)
{
    vx_status status = VX_SUCCESS;
    ExynosVpuTaskIf *task_if_0;
...
ExynosVpuTaskIf*
ExynosVpuKernelPhase::initTask0FromApi(void)
{
    vx_status status = VX_SUCCESS;
    ...
    ExynosVpuPu *dma_out = pu_factory.createPu(mde_subchain, VPU_PU_DMAOT_WIDE0);
    dma_out->setSize(iosize, iosize);

    ExynosVpuPu::connect(dma_in1, 0, mde, 0);
    ExynosVpuPu::connect(dma_in2, 0, mde, 1);
    ExynosVpuPu::connect(mde, 1, dma_out, 0);
    ...
    task_if->setIoPu(VS4L_DIRECTION_IN, 0, dma_in1);
    ...
    task_if->setIoPu(VS4L_DIRECTION_IN, 1, dma_in2);
...
vx_status
ExynosVpuKernelPhase::updateTaskParamFromVX(vx_node node, const vx_reference *parameters)
{
    vx_status status = VX_SUCCESS;
...
vx_status
ExynosVpuKernelPhase::initVxIo(const vx_reference *parameters)
{
    vx_status status = VX_SUCCESS;
