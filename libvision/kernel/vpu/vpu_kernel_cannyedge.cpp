...
static map<void*, ExynosVpuKernelCannyEdge*> vpu_kernel_cannyedge_node_map;
...
static vx_status vxCannyEdgeKernel(vx_node node, const vx_reference parameters[], vx_uint32 num)
{
    ...
    vx_status status;
    ExynosVpuKernelCannyEdge *vpu_kernel_accum = vpu_kernel_cannyedge_node_map[(void*)node];
...
{
    EXYNOS_VPU_KERNEL_IF_IN();
    vx_status status = ExynosVpuKernelCannyEdge::inputValidator(node, index);
    if (status != VX_SUCCESS) {
        VXLOGE("input validation fail, index:%d, err:%d", index, status);
    }
    EXYNOS_VPU_KERNEL_IF_OUT();
...
{
    EXYNOS_VPU_KERNEL_IF_IN();
    vx_status status = ExynosVpuKernelCannyEdge::outputValidator(node, index, meta);
    if (status != VX_SUCCESS) {
        VXLOGE("output validation fail, index:%d, err:%d", index, status);
    }
    EXYNOS_VPU_KERNEL_IF_OUT();
...
static vx_status vxCannyEdgeInitializer(vx_node node, const vx_reference *parameters, vx_uint32 num)
{
    ...
    ExynosVpuKernelCannyEdge *cannyedge;
    ...
    if (vpu_kernel_cannyedge_node_map[(void*)node] != NULL) {
        VXLOGE("already allocated kernel");
        status = VX_ERROR_NOT_ALLOCATED;
        goto EXIT;
    } else {
        cannyedge = new ExynosVpuKernelCannyEdge(cannyedge_vpu_kernel.name, dimof(cannyedge_kernel_params));
    }
    ...
    vpu_kernel_cannyedge_node_map[(void*)node] = cannyedge;
...
static vx_status vxCannyEdgeDeinitializer(vx_node node, const vx_reference *parameters, vx_uint32 num)
{
    ...
    if (vpu_kernel_cannyedge_node_map[(void*)node] != NULL)
        delete vpu_kernel_cannyedge_node_map[(void*)node];

    vpu_kernel_cannyedge_node_map.erase((void*)node);
...
vx_kernel_description_t cannyedge_vpu_kernel = {
    VX_KERNEL_CANNY_EDGE_DETECTOR,
    "com.samsung.vpu.canny_edge_detector",
    vxCannyEdgeKernel,
