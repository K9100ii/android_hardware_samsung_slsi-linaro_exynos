...
static map<void*, ExynosVpuKernelConvolution*> vpu_kernel_node_map;
...
static vx_status vxConvolutionKernel(vx_node node, const vx_reference parameters[], vx_uint32 num)
{
    ...
    vx_status status;
    ExynosVpuKernelConvolution *vpu_kernel = vpu_kernel_node_map[(void*)node];
    if (vpu_kernel == NULL) {
...
{
    EXYNOS_VPU_KERNEL_IF_IN();
    vx_status status = ExynosVpuKernelConvolution::inputValidator(node, index);
    if (status != VX_SUCCESS) {
...
{
    EXYNOS_VPU_KERNEL_IF_IN();
    vx_status status = ExynosVpuKernelConvolution::outputValidator(node, index, meta);
    if (status != VX_SUCCESS) {
...
static vx_status vxConvolutionInitializer(vx_node node, const vx_reference *parameters, vx_uint32 num)
{
    ...
    ExynosVpuKernelConvolution *convolution;
    ...
    if (vpu_kernel_node_map[(void*)node] != NULL) {
        ...
        convolution = new ExynosVpuKernelConvolution(convolution_vpu_kernel.name, dimof(convolution_kernel_params));
    }
    ...
    status = convolution->setupDriver();
    if (status != VX_SUCCESS) {
    ...
    vpu_kernel_node_map[(void*)node] = convolution;
...
vx_kernel_description_t convolution_vpu_kernel = {
    VX_KERNEL_CUSTOM_CONVOLUTION,
    "com.samsung.vpu.VX_KERNEL_CUSTOM_CONVOLUTION",
    vxConvolutionKernel,
