...
#define LOG_TAG "KeymasterHidlTest"
#include <cutils/log.h>

int strongbox_test = true;
...
void KeymasterHidlTest::InitializeKeymaster(sp<IKeymasterDevice> keymaster) {
    ...
    ALOGE("[TEST] securityLevel: %d", securityLevel_);
    ALOGE("[TEST] name_: %s", name_.c_str());
/*
    if (strongbox_test) {
        ALOGE("KJT:: run strongbox VTS\n");
        keymaster->getHardwareInfo([&](SecurityLevel securityLevel, const hidl_string& name,
                    const hidl_string& author) {
            if(securityLevel == SecurityLevel::STRONGBOX) {
                ALOGE("KJT: replace default test Keymaster to strongbox");
                keymaster_ = keymaster;
                securityLevel_ = securityLevel;
                name_ = name;
                author_ = author;
