...
/*
 * This VTS is based on AOSP Keymaster VTS.
 * Added some cases and command options for Strongbox.
 */
    ...
    //if (0) {
        for (int i = 0; i < att_hw_enforced.size(); i++) {
            if (att_hw_enforced[i].tag == TAG_BOOT_PATCHLEVEL ||
                ...
                ALOGE("KJT: %s:: %d. date(%s)\n", __func__, __LINE__, date.c_str());
                date.insert(6, "-");
                date.insert(4, "-");
...
/* ExySp */
/*
 * SigningOperationsTest.RsaAbortMany
 *
 * Verifies that operations can be aborted correctly.  Uses an RSA signing operation for the test,
 * but the behavior should be algorithm and purpose-independent.
 */
TEST_P(SigningOperationsTest, RsaAbortMany) {
    ASSERT_EQ(ErrorCode::OK, GenerateKey(AuthorizationSetBuilder()
                                             .RsaSigningKey(2048, 65537)
                                             .Digest(Digest::NONE)
                                             .Authorization(TAG_NO_AUTH_REQUIRED)
...
#define MAX_SESSION_COUNT 4
    OperationHandle op_handle[MAX_SESSION_COUNT] = {0,};
    AuthorizationSet out_params[MAX_SESSION_COUNT];
        ...
        for (int i = 0; i < MAX_SESSION_COUNT; i++) {
            ASSERT_EQ(ErrorCode::OK,
                      Begin(KeyPurpose::SIGN, key_blob_,
                            AuthorizationSetBuilder().Digest(Digest::NONE).Padding(PaddingMode::NONE),
                            &out_params[i], &op_handle[i]));
        }

        for (int i = 0; i < MAX_SESSION_COUNT; i++) {
            EXPECT_EQ(ErrorCode::OK, Abort(op_handle[i]));

            // Another abort should fail
            EXPECT_EQ(ErrorCode::INVALID_OPERATION_HANDLE, Abort(op_handle[i]));
...
/* ExySp */
auto invalid_wrapped_key = hex2str(
    "3082017902010004820100A0E69B1395D382354FC0E7F74AC068C5818279D76D46745C7274997D045BAA8B9763B3F3"
    "A9E5E59ECA99273AAAE0A37449DA9B1E67B66EC4E42BB62C25346683A43A9F2ACBCA6D350B25551CC53CE0721D29BE"
...
/* ExySp */
TEST_P(ImportWrappedKeyTest, InvalidWappedKey) {
    auto wrapping_key_desc = AuthorizationSetBuilder()
                                 .RsaEncryptionKey(2048, 65537)
                                 .Digest(Digest::SHA1)
                                 .Padding(PaddingMode::RSA_OAEP)
    ...
    ASSERT_EQ(ErrorCode::UNKNOWN_ERROR,
              ImportWrappedKey(
                  invalid_wrapped_key, wrapping_key, wrapping_key_desc, zero_masking_key,
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (argv[i][0] == '-') {
            ...
            if (std::string(argv[i]) == "--tee") {
                strongbox_test = false;
            } else {
                strongbox_test = true;
            }
        }
