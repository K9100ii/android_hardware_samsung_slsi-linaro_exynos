...
/* for supporting keymaster 4.1 method */
#define CMD_SSP_DEVICE_LOCKED                   0x0013
#define CMD_SSP_EARLY_BOOT_ENDED                0x0014
/* for keymint 1.0 method */
#define CMD_SSP_SELF_ATTEST_KEY                 0x0015
#define CMD_SSP_FACTORY_ATTEST_KEY              0x0016
...
/* support up to 2048 bits HMAC key -> 512*/
#define SSP_HMAC_MAX_KEY_SIZE (512)
...
/* confirmation buffer length should be 32 */
#define SSP_CONFIRMATION_TOKEN_LEN		32
...
#define SSP_ATTEST_ISSUER_NAME_MAX_LEN (64) /* OID length */
...
typedef struct __attribute__((packed, aligned(8))) {
    ...
    hw_auth_token_t auth_token __attribute__((aligned(8)));        /* in */
    hw_timestamp_token_t timestamp_token;                     /* in */
    uint32_t authtoken_info;					                    /* in */
    uint32_t in_params_len;                                         /* in */
...
typedef struct __attribute__((packed, aligned(8))) {
    ...
    hw_timestamp_token_t timestamp_token;                     /* in */
    uint32_t authtoken_info;					                    /* in */
    uint32_t confirmation_token_len;								/* in */
    uint32_t input_len;                                             /* in */
    uint32_t signature_len;                                         /* in */
    uint32_t input_consumed_len;                                    /* out */
    uint32_t output_len;                                            /* out */
    uint32_t output_result_len;                                     /* out */
    uint8_t confirmation_token[SSP_CONFIRMATION_TOKEN_LEN];   /* in */
...
typedef struct __attribute__((packed, aligned(4))) {
    ...
    uint32_t attestation_blob_len;              /* in */
    uint32_t issuer_blob_len;					/* in */
    uint8_t issuer_blob[SSP_ATTEST_ISSUER_NAME_MAX_LEN];      	/* in */
    uint8_t attestation_blob[SSP_ATTESTATION_DATA_MAX_LEN];     /* in */
    uint8_t keyblob[SSP_KEY_BLOB_MAX_LEN];                      /* in */
...
/*
 * device_locked_t structure : for supporting keymaster 4.1 method
 * @ needs to be aligned 8bytes for uint64_t variables; timestamp_token.timestamp
 */
typedef struct __attribute__((packed, aligned(8))) {
...
/* early_boot_ended_t structure */
typedef struct __attribute__((packed, aligned(4))) {
    uint32_t early_boot_status;                       /* out */
} early_boot_ended_t;
...
typedef struct __attribute__((packed, aligned(4))) {
    ...
    union {
        ...
        early_boot_ended_t early_boot_ended;
