...
ErrorCode KeyMintAidlTestBase::ImportKey(const AuthorizationSet& key_desc,
                                         ...
                                         KeyFormat format,
                                         const string& key_material) {
    return ImportKey(key_desc, attest_key,
                     format, key_material, &key_blob_, &key_characteristics_);
}
...
ErrorCode KeyMintAidlTestBase::Begin(KeyPurpose purpose,
                                     const vector<uint8_t>& key_blob,
                                     const AuthorizationSet& in_params,
                                     const HardwareAuthToken& hw_token,
                                     AuthorizationSet* out_params)
{
    SCOPED_TRACE("Begin");
    Status result;
    BeginResult out;
    result = keymint_->begin(purpose, key_blob, in_params.vector_data(), hw_token, &out);
    if (result.isOk()) {
        *out_params = out.params;
        challenge_ = out.challenge;
        op_ = out.operation;
...
auto KeyMintAidlTestBase::ProcessMessage(const vector<uint8_t>& key_blob,
...
string KeyMintAidlTestBase::ProcessMessage(const vector<uint8_t>& key_blob,
                                           KeyPurpose operation,
                                           const string& message,
                                           const AuthorizationSet& in_params,
                                           AuthorizationSet* out_params) {
...
void verify_serial(X509* cert, const uint64_t expected_serial) {
    ...
    BIGNUM *bn;
    bn = ASN1_INTEGER_to_BN(X509_get_serialNumber(cert), ser.get());
    EXPECT_TRUE(bn);
