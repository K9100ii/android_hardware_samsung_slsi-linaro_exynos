...
TEST_P(ImportKeyTest, EcdsaAttestationIDFailed) {
    ...
    for (const KeyParameter& tag : attestation_id_tags) {
        ...
        // include an attestation ID field.
        ...
        ASSERT_EQ(ErrorCode::CANNOT_ATTEST_IDS, ImportKey(builder,
                                                          KeyFormat::PKCS8,
                                                          ec_256_key));
    }
}

TEST_P(ImportKeyTest, EcdsaWithFactoryAttestKey) {
    ASSERT_EQ(ErrorCode::OK, ImportKey(AuthorizationSetBuilder()
                                                  .EcdsaSigningKey(EcCurve::P_256)
                                                  .Digest(Digest::SHA_2_256)
                                                  .Authorization(TAG_NO_AUTH_REQUIRED)
...
TEST_P(ImportKeyTest, EcdsaWithProvidedAttestKey) {
...
TEST_P(UnlockedDeviceRequiredTest, LockUnlockWithoutPasswordOnly) {
    auto [aesKeyData, hmacKeyData, rsaKeyData, ecdsaKeyData] =
        CreateTestKeys(TAG_UNLOCKED_DEVICE_REQUIRED, ErrorCode::OK);

    HardwareAuthToken auth_token;
    auth_token.challenge = 0x00;
    auth_token.timestamp.milliSeconds = getTime();

    EXPECT_EQ(ErrorCode::OK, UseAesKey(aesKeyData.blob, auth_token));
    EXPECT_EQ(ErrorCode::OK, UseHmacKey(hmacKeyData.blob, auth_token));
    EXPECT_EQ(ErrorCode::OK, UseRsaKey(rsaKeyData.blob, auth_token));
    EXPECT_EQ(ErrorCode::OK, UseEcdsaKey(ecdsaKeyData.blob, auth_token));

    //VerificationToken token;
    TimeStampToken token;
    token.challenge = 0x00;
    token.timestamp.milliSeconds = getTime();

    ErrorCode rc = GetReturnErrorCode(
            keyMint().deviceLocked(false /* passwordOnly */, token /* timestampToken */));
...
TEST_P(UnlockedDeviceRequiredTest, LockUnlockWithPasswordOnly) {
    auto [aesKeyData, hmacKeyData, rsaKeyData, ecdsaKeyData] =
        CreateTestKeys(TAG_UNLOCKED_DEVICE_REQUIRED, ErrorCode::OK);
    ...
    /* TEST for password only parameter - true */
    /* LOCK with password only mode */
    // VerificationToken token;
    TimeStampToken token;
    token.challenge = 0x00;
    token.timestamp.milliSeconds = getTime();
    ...
    ErrorCode rc = GetReturnErrorCode(
            keyMint().deviceLocked(true /* passwordOnly */, token /* timestampToken */));
    ...
    /* cannt UNLOCK without HW_AUTH_PASSWORD TOKEN */
    auth_token.challenge = 0x00;
    auth_token.authenticatorType = HardwareAuthenticatorType::NONE;
    auth_token.timestamp.milliSeconds = getTime();

    EXPECT_EQ(ErrorCode::DEVICE_LOCKED, UseAesKey(aesKeyData.blob, auth_token));
    EXPECT_EQ(ErrorCode::DEVICE_LOCKED, UseHmacKey(hmacKeyData.blob, auth_token));
    EXPECT_EQ(ErrorCode::DEVICE_LOCKED, UseRsaKey(rsaKeyData.blob, auth_token));
    EXPECT_EQ(ErrorCode::DEVICE_LOCKED, UseEcdsaKey(ecdsaKeyData.blob, auth_token));

    /* UNLOCK with HW_AUTH_PASSWORD TOKEN */
    auth_token.authenticatorType = HardwareAuthenticatorType::PASSWORD;
    auth_token.timestamp.milliSeconds = getTime();
    ...
    /* TEST for password only parameter - false */
    token.challenge = 0x00;
    token.timestamp.milliSeconds = getTime();

    rc = GetReturnErrorCode(
            keyMint().deviceLocked(false /* passwordOnly */, token /* timestampToken */));
    ASSERT_EQ(ErrorCode::OK, rc);
